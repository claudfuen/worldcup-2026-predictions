...
export async function GET() {
  try {
    ...
    const posByKey = new Map(
      (data.awards.players ?? []).map((p) => [
        `${p.name}|${p.teamCode}`,
        p.position,
      ])
    )
    const players = playerUniverse(data.awards).map((p) => ({
      name: p.player,
      team: p.teamCode,
      slug: p.slug,
      pos: posByKey.get(`${p.player}|${p.teamCode}`) ?? "",
    }))
    ...
    const suggest = {
      ...
      players: data.awards.goldenBoot
        .filter((e) => e.goals > 0)
        .slice(0, 6)
        .map((e) => playerSlug(e.player, e.teamCode)),
    }
    return NextResponse.json(
      { matches: out, players, suggest },
