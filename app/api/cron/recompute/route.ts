...
export async function GET(req: Request) {
  ...
  if (new Date().toISOString().slice(0, 10) > STOP_RECOMPUTE_AFTER) {
    return NextResponse.json({
      ok: true,
      skipped: "tournament complete; predictions frozen",
    })
  }
  ...
  const prevSig = await kvGetJSON<{ sig: string; at: number }>(
    LIVE_SIG_KEY
  ).catch(() => null)
  ...
  let baseline = await kvGetJSON<BaselineSnapshot>(BASELINE_KEY).catch(
    () => null
  )
