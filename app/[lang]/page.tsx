...
export default async function Page() {
  const t = await getT()
  const [data, live, awards] = await Promise.all([
    getPredictions(),
    getLiveMatches(),
    getLiveAwards(),
  ])
  ...
  const groups = hasLive
    ? finalizeGroups(data.groups, matches, ratings)
    : data.groups
  ...
  for (const p of computeWatchability(
    matches,
    data.teams,
    groups
  ).byMatch.values()) {
    ...
    <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <header>
        <MastheadVerdict
          ...
          finalMatch={lMatches.find((mm) => mm.round === "FINAL")}
        />
      </header>
      <TournamentStage
        matches={lMatches}
        matchesPlayed={groupPlayed}
        totalGroupMatches={data.totalGroupMatches}
        className="mt-6"
      />
