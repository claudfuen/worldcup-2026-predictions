...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title, description, url: localeHref(locale, "/calendar"), type: "website" },
...
export default async function CalendarPage() {
  ...
  const matches = hasLive
    ? finalizeBracket(overlaid, finalizeGroups(data.groups, overlaid, ratings), ratings)
    : overlaid;
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <header className="mb-6 max-w-3xl">
        <div className="text-primary font-mono text-xs font-semibold tracking-wide uppercase">{t("calendar.eyebrow")}</div>
        <h1 className="mt-1 text-3xl font-semibold tracking-tight text-balance sm:text-4xl">{t("calendar.heading")}</h1>
        <p className="text-muted-foreground mt-2 text-base text-pretty">{t("calendar.lede")}</p>
      </header>
