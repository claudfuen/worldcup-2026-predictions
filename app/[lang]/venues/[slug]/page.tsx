...
export async function generateMetadata({
  ...
  const description = t("venues.detailMetaDesc", {
    venue: v.fifaName,
    city: v.city,
  })
  return {
    ...
    openGraph: {
      title,
      description,
      url: localeHref(locale, `/venues/${slug}`),
      type: "website",
    },
...
export default async function VenuePage({
  ...
  const roundsHosted = ROUND_ORDER.filter((r) =>
  ...
  const stats: { label: string; value: string }[] = [
    ...
    {
      label: t("venues.statGoals"),
      value: played.length ? String(goals) : "—",
    },
    ...
    <main className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <Breadcrumbs
        items={[
          ...
          { label: t("nav.stadiums"), href: localeHref(locale, "/venues") },
          { label: v.fifaName },
        ]}
      />
      <header className="mt-3 mb-6">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1.5">
          ...
          {hostsFinal && (
            <span className="shrink-0 rounded-md border border-contention/40 bg-contention/10 px-2 py-0.5 font-mono text-[10px] font-semibold tracking-wide text-contention uppercase">
              {t("venues.hostsFinal")}
            </span>
          )}
        </div>
        <div className="mt-2 flex flex-wrap items-center gap-x-2.5 gap-y-1 text-sm text-muted-foreground">
          ...
          <span className="inline-flex items-center gap-1.5">
            <Flag code={v.hostCode} size={16} /> {v.city}, {hostName}
          </span>
        </div>
        {roundsHosted.length > 0 && (
          <div className="mt-3 flex flex-wrap items-center gap-1.5">
            <span className="me-1 font-mono text-[10px] font-semibold tracking-wide text-muted-2 uppercase">
              {t("venues.hosts")}
            </span>
            {roundsHosted.map((r) => (
              <span
                key={r}
                className={`rounded px-1.5 py-0.5 font-mono text-[9px] font-semibold tracking-wide uppercase ${r === "FINAL" ? "bg-contention/10 text-contention" : "bg-muted/50 text-muted-foreground"}`}
              >
                {t(ROUND_SHORT[r])}
              </span>
            ))}
      ...
      {photo && (
        <figure className="mb-6 overflow-hidden rounded-2xl border border-border dark:inset-ring dark:inset-ring-white/5">
          <img
            src={photo.url}
          ...
          />
          <figcaption className="flex flex-wrap items-center justify-end gap-x-1.5 bg-card px-3 py-1.5 text-[10px] text-muted-2">
            <a
              href={photo.source}
              target="_blank"
      ...
      <dl className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-border bg-card sm:grid-cols-4 dark:inset-ring dark:inset-ring-white/5">
        {stats.map((s) => (
          ...
          >
            <dt className="text-[10px] font-medium tracking-wide text-muted-2 uppercase">
              {s.label}
            </dt>
      ...
      <h2 className="mt-8 mb-3 font-mono text-xs font-semibold tracking-wide text-muted-foreground uppercase">
        {t("venues.matchesHeading")}
      </h2>
      <div className="divide-y divide-border/50 overflow-hidden rounded-2xl border border-border bg-card dark:inset-ring dark:inset-ring-white/5">
        {matches.map((m) => (
          <VenueMatchRow key={m.match} m={m} locale={locale} t={t} />
        ))}
      </div>
      ...
      <RelatedLinks
        links={[
          {
            label: t("venues.relatedAllVenues"),
            href: localeHref(locale, "/venues"),
          },
          {
...
function side(m: MatchInfo, which: "home" | "away") {
  ...
  return {
    code: code ?? proj?.code ?? null,
    name: name ?? proj?.name ?? slot ?? null,
    resolved: !!code,
  }
}
...
function VenueMatchRow({
  ...
  return (
    <Link
      href={localeHref(locale as never, `/match/${m.match}`)}
      className="flex items-center gap-3 px-4 py-2.5 transition-colors hover:bg-muted/20"
    >
      <div className="w-16 shrink-0 text-[11px] text-muted-2">
      ...
      <div className="shrink-0 text-right">
        {final || live ? (
          <div className="space-y-0.5 font-mono text-sm tabular-nums">
            <div
              className={
                homeWin ? "font-bold text-foreground" : "text-muted-foreground"
              }
            >
              {m.homeScore}
              {ps && (
                <span className="ms-0.5 text-[11px] font-normal text-muted-2">
                  ({ps.home})
                </span>
              )}
            </div>
            <div
              className={
                awayWin ? "font-bold text-foreground" : "text-muted-foreground"
              }
            >
              {m.awayScore}
              {ps && (
          ...
          >
            <LocalTime utc={m.utc} mode="timeshort" />
          </div>
        )}
        <div
          className={`mt-0.5 font-mono text-[10px] font-semibold tracking-wide uppercase ${live ? "text-live" : final ? "text-win" : "text-muted-2"}`}
        >
          {live
            ? (m.liveDetail ?? t("common.live"))
            : final
              ? t("scoreTicker.ft")
              : ""}
        </div>
...
function Row({
  ...
  return (
    <span className="flex items-center gap-2">
      ...
      <span
        className={`min-w-0 truncate ${win ? "font-bold" : resolved ? "font-medium" : "text-foreground/70"}`}
      >
        {name ?? "—"}
      </span>
    </span>
