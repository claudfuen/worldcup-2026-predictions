...
export default async function Image({
  ...
  const [flag, photo] = await Promise.all([
    flagDataUri(v.hostCode),
    imgDataUri(photoMeta?.url),
  ])
      ...
      >
        ...
        <div
          style={{
            ...
            display: "flex",
            background:
              "linear-gradient(180deg, rgba(6,10,8,0.74) 0%, rgba(6,10,8,0.10) 34%, rgba(6,10,8,0.55) 66%, rgba(6,10,8,0.93) 100%)",
          }}
        />
        <div
          style={{
        ...
        >
          Photo: {photoMeta.artist}
          {photoMeta.license ? ` · ${photoMeta.license}` : ""}
        </div>
      </div>,
      { ...size }
    )
  ...
  return new ImageResponse(
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", color: "#8aa394", fontSize: 26 }}>
          worldcup2026predictions.app
        </div>
      </div>
    </div>,
