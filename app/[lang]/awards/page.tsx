...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const description = lead?.clinched
    ...
    : lead
      ? t("awards.metaDescLeader", {
          player: lead.player,
          goals: lead.goals,
          pct: forecastPct(lead.winProb),
        })
      : t("awards.metaDesc")
  return {
...
export default async function AwardsPage() {
  const t = await getT()
  const locale = await getLocale()
  const [data, awards, live] = await Promise.all([
    getPredictions(),
    getLiveAwards(),
    getLiveMatches(),
  ])
  ...
  const leadCount = leader
    ? goldenBoot.filter((e) => e.goals === leader.goals).length
    : 0
  ...
  const verdict = !leader
    ...
    : leader.clinched
      ? clinchedCount > 1
        ? t("awards.verdictWonShared", {
            player: leader.player,
            goals: leader.goals,
          })
        : t("awards.verdictWon", { player: leader.player, goals: leader.goals })
      : leadCount > 1
        ? t("awards.verdictTied", {
            player: leader.player,
            n: leadCount - 1,
            goals: leader.goals,
          })
        : t("awards.verdictLeader", {
            player: leader.player,
            goals: leader.goals,
            pct: forecastPct(leader.winProb),
          })
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <Section
        title={t("awards.playmaker")}
        desc={t("awards.playmakerDesc")}
        className="mt-10"
      >
      ...
      <p className="mt-6 text-xs text-pretty text-muted-2">
        {data.complete
          ? t("awards.matchesNoteFinal", { n: matchesCounted })
          : t("awards.matchesNote", { n: matchesCounted, iters: "20k" })}{" "}
        {!data.complete && t("awards.footnote")}
      </p>
...
function Section({
  ...
  return (
    <section className={className}>
      ...
      <p className="mt-0.5 mb-3 text-xs text-pretty text-muted-2">{desc}</p>
      {children}
    </section>
  )
}
