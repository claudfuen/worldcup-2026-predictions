...
export async function generateMetadata({
  ...
  try {
    ...
    const home = m.home
      ? t(`teams.${m.home}`)
      : m.slotHome
        ? prettySlot(t, m.slotHome)
        : t("common.tbd")
    const away = m.away
      ? t(`teams.${m.away}`)
      : m.slotAway
        ? prettySlot(t, m.slotAway)
        : t("common.tbd")
    ...
    return {
      ...
      openGraph: {
        title,
        description,
        url: localeHref(locale, `/match/${m.match}`),
        type: "article",
      },
...
export default async function MatchPage({
  ...
  const eloRanked = [...data.teams].sort(
    (a, b) => (b.ratingExact ?? b.rating) - (a.ratingExact ?? a.rating)
  )
  ...
  const fifaRankOf = (code?: string | null): number | undefined =>
    code ? FIFA_RANK[code] : undefined
  ...
  const initial: MatchLivePayload = {
    m: foundRaw,
    summary,
    liveProbs,
    proj,
  ...
  for (const e of summary.events) {
    ...
    for (const nm of [e.player, e.assist, e.playerOff]) {
      if (nm)
        tlPlayers.set(`${e.teamCode}|${nm}`, { name: nm, code: e.teamCode })
    }
  }
  ...
  const heat = computeWatchability(all, data.teams, data.groups).byMatch.get(
    m.match
  )
  ...
  const proseText =
    state === "defined" && m.probs
      ? predictionProse(t, m, homePred?.rating, awayPred?.rating)
      : null
  ...
  const crumbs =
    m.round === "GROUP" && m.group
      ? [
          { label: t("match.crumbHome"), href: localeHref(locale, "/") },
          { label: t("nav.groups"), href: localeHref(locale, "/groups") },
          {
            label: t("match.groupLabel", { group: m.group }),
      ...
      : [
          ...
          {
            label: roundName(t, m.round),
            href: localeHref(locale, "/bracket"),
          },
          { label: matchLabel },
        ]
  const groupView = m.group
    ? groups.find((g) => g.group === m.group)
    : undefined
  ...
  if (m.home && m.homeName)
    exploreLinks.push({
      label: t(`teams.${m.home}`),
      href: localeHref(locale, `/team/${slugForCode(m.home)}`),
      code: m.home,
    })
  if (m.away && m.awayName)
    exploreLinks.push({
      label: t(`teams.${m.away}`),
      href: localeHref(locale, `/team/${slugForCode(m.away)}`),
      code: m.away,
    })
  exploreLinks.push({
    label: t("match.fullSchedule"),
    href: localeHref(locale, "/schedule"),
  })
  exploreLinks.push({
    label: t("match.howItWorks"),
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <MatchHero
        ...
        homeRank={eloRankOf(m.home)}
        awayRank={eloRankOf(m.away)}
        homeFifa={fifaRankOf(m.home)}
        awayFifa={fifaRankOf(m.away)}
      />
...
function predictionProse(
  ...
  const scoreStr = top
    ? " " +
      t(m.xg ? "match.proseScoreXg" : "match.proseScore", {
        ...
        a: top.a,
        ...(m.xg
          ? { xgHome: m.xg.home.toFixed(1), xgAway: m.xg.away.toFixed(1) }
          : {}),
      })
    : ""
  let whyStr = ""
  if (homeRating != null && awayRating != null) {
    ...
    whyStr =
      " " +
      (abs >= 25
        ? t("match.proseWhyEdge", {
            stronger: gap >= 0 ? m.homeName : m.awayName,
            elo: abs,
          })
