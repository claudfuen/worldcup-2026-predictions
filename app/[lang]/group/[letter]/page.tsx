...
export default async function GroupPage({
  ...
  const groups = localizeGroups(
    hasLive
      ? finalizeGroups(data.groups, overlaid, ratingsFromTeams(data.teams))
      : data.groups,
    t
  )
  ...
  const contending = gv.teams
    .filter((t) => t.status === "live")
    .sort((a, b) => b.advance - a.advance)
  const verdict: string[] = []
  if (through.length)
    verdict.push(
      t("groupDetail.verdictQualified", {
        teams: nameList(through),
        count: through.length,
      })
    )
  if (contending[0] && contending[0].advance > 0.02)
    verdict.push(
      t("groupDetail.verdictContending", {
        team: contending[0].name,
        pct: pct(contending[0].advance),
      })
    )
  if (out.length)
    verdict.push(t("groupDetail.verdictOut", { teams: nameList(out) }))
  ...
  if (gi > 0)
    related.push({
      label: t("groupDetail.groupLabel", { group: GROUPS[gi - 1] }),
      href: localeHref(locale, `/group/${GROUPS[gi - 1].toLowerCase()}`),
    })
  ...
  related.push({
    label: t("groupDetail.fullScheduleLink"),
    href: localeHref(locale, "/schedule"),
  })
  related.push({
    label: t("groupDetail.howItWorksLink"),
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <header className="mt-3 max-w-3xl">
        ...
        <div className="mt-4">
          <ShareBar
            text={t("groupDetail.shareText", {
              group: L,
              lead: leader
                ? t("groupDetail.shareLead", { team: leader.name })
                : t("groupDetail.shareLive"),
            })}
            path={`/group/${letter.toLowerCase()}`}
          />
        </div>
      </header>
      ...
      <div className="mt-8 grid items-start gap-6 lg:grid-cols-5">
        <section className="lg:col-span-3">
          <h2 className="mb-3 font-mono text-xs font-semibold tracking-wide text-muted-foreground uppercase">
            {t("groupDetail.standingsHeading")}
          </h2>
          <div className="overflow-hidden rounded-2xl border border-border bg-card">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-[10px] tracking-wide text-muted-foreground">
                  <th className="py-2 pr-1 pl-3 text-left font-medium">
                    {t("groupDetail.colTeam")}
                  </th>
                  <th
                    className="w-6 px-1 text-center font-medium"
                    title={t("groupDetail.colPlayedTitle")}
                  >
                    {t("groupDetail.colPlayed")}
                  </th>
                  <th
                    className="w-6 px-1 text-center font-medium"
                    title={t("groupDetail.colWonTitle")}
                  >
                    {t("groupDetail.colWon")}
                  </th>
                  <th
                    className="w-6 px-1 text-center font-medium"
                    title={t("groupDetail.colDrawnTitle")}
                  >
                    {t("groupDetail.colDrawn")}
                  </th>
                  <th
                    className="w-6 px-1 text-center font-medium"
                    title={t("groupDetail.colLostTitle")}
                  >
                    {t("groupDetail.colLost")}
                  </th>
                  <th
                    className="hidden w-7 px-1 text-center font-medium sm:table-cell"
                    title={t("groupDetail.colGoalsForTitle")}
                  >
                    {t("groupDetail.colGoalsFor")}
                  </th>
                  <th
                    className="hidden w-7 px-1 text-center font-medium sm:table-cell"
                    title={t("groupDetail.colGoalsAgainstTitle")}
                  >
                    {t("groupDetail.colGoalsAgainst")}
                  </th>
                  <th
                    className="w-7 px-1 text-center font-medium"
                    title={t("groupDetail.colGoalDiffTitle")}
                  >
                    {t("groupDetail.colGoalDiff")}
                  </th>
                  <th
                    className="w-7 px-1 text-center font-semibold"
                    title={t("groupDetail.colPointsTitle")}
                  >
                    {t("groupDetail.colPoints")}
                  </th>
                  <th
                    className="w-16 px-1 pr-3 text-right font-medium"
                    title={t("groupDetail.colAdvanceTitle")}
                  >
                    {t("groupDetail.colAdvance")}
                  </th>
                </tr>
              </thead>
              <tbody>
                {gv.teams.map((tm, i) => {
                  ...
                  return (
                    <tr
                      key={tm.code}
                      className={`border-b border-l-2 border-border/40 last:border-b-0 ${zone} ${elim ? "opacity-45" : ""}`}
                    >
                      <td className="py-2 pr-1 pl-2.5">
                        ...
                        >
                          ...
                          <span
                            className={`truncate text-[13px] font-medium ${elim ? "line-through" : ""}`}
                          >
                            {tm.name}
                          </span>
                        </Link>
          ...
          <div className="divide-y divide-border/50 overflow-hidden rounded-2xl border border-border bg-card">
            {fixtures.map((m) => {
              ...
              return (
                ...
                >
                  <span className="w-14 shrink-0 font-mono text-[11px] text-muted-2 sm:w-24">
      ...
      <ExploreSection title={t("groupDetail.exploreTitle")} links={related}>
        ...
        <BracketTeaser
          matches={localizeMatches(overlaid, t)}
          teams={localizeTeams(data.teams, t)}
        />
...
function Cell({
  ...
  return (
    <td
      className={`px-1 text-center font-mono text-xs tabular-nums ${muted ? "text-muted-foreground" : ""} ${cls ?? ""}`}
    >
      {v}
    </td>
  )
