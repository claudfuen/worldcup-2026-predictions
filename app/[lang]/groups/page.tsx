...
export default async function GroupsPage() {
  ...
  const verdict =
    decidedCount === groups.length
      ...
      : t("groups.verdictPartial", {
          settled: decidedCount,
          total: groups.length,
          through: qualified,
        })
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        {localizedGroups.map((g) => (
          <GroupCard
            ...
            teams={g.teams}
            decided={g.decided}
            prov={provByGroup[g.group]}
          />
        ))}
      </div>
      <Legend t={t} />
      <p className="mt-3 max-w-3xl text-xs text-muted-2">
      ...
      <RelatedLinks
        links={[
          ...
          },
          {
            ...
            hint: t("groups.linkOverviewHint"),
          },
        ]}
      />
    </main>
...
function GroupCard({
  ...
  teams: GroupTeamView[]
  decided: boolean
  prov?: ProvisionalGroup | null
}) {
  const live = !!prov
  return (
    <div
      className={`overflow-hidden rounded-2xl border bg-card ${live ? "border-live/40" : "border-border"}`}
    >
      <div className="flex items-center justify-between border-b border-border/60 px-4 py-2.5">
        ...
        {live ? (
          ...
          <span
            className={`font-mono text-[10px] font-semibold tracking-wide uppercase ${decided ? "text-win" : "text-muted-foreground"}`}
          >
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-[10px] tracking-wide text-muted-foreground">
            <th className="py-1.5 pr-1 pl-3 text-left font-medium">
              {t("groups.colTeam")}
            </th>
            <th
            ...
            <th
              className="w-6 px-1 text-center font-medium"
              title={t("groups.colWonTitle")}
            >
              {t("groups.colWon")}
            </th>
            <th
              className="w-6 px-1 text-center font-medium"
              title={t("groups.colDrawnTitle")}
            >
              {t("groups.colDrawn")}
            </th>
            <th
              className="w-6 px-1 text-center font-medium"
              title={t("groups.colLostTitle")}
            >
              {t("groups.colLost")}
            </th>
            <th
              className="hidden w-7 px-1 text-center font-medium sm:table-cell"
              title={t("groups.colGfTitle")}
            >
              {t("groups.colGf")}
            </th>
            <th
              className="hidden w-7 px-1 text-center font-medium sm:table-cell"
              title={t("groups.colGaTitle")}
            >
              {t("groups.colGa")}
            </th>
            <th
            ...
            <th
              className="w-14 px-1 pr-3 text-right font-medium"
              title={t("groups.colAdvTitle")}
            >
              {t("groups.colAdv")}
            </th>
          </tr>
        ...
        <tbody>
          {teams.map((team, i) => (
            <Row
              ...
              team={team}
              pos={i + 1}
              cut={i === 1 ? "qualify" : i === 2 ? "third" : null}
            />
          ))}
        </tbody>
      </table>
      {teams.some((team) => team.need) && (
        <div className="space-y-1.5 border-t border-border/60 px-4 py-3">
          {teams
            .filter((team) => team.need)
            .map((team) => (
              <div key={team.code} className="flex items-center gap-2 text-xs">
                <Link
                  href={localeHref(locale, `/team/${slugForCode(team.code)}`)}
...
function Row({
  ...
  const cutBorder =
    cut === "qualify"
      ? "border-b-primary/50 border-b border-dashed"
      : cut === "third"
        ? "border-b-border border-b border-dotted"
        : ""
  return (
    <tr
      className={`border-l-2 ${zone} ${cutBorder} ${elim ? "opacity-45" : ""}`}
    >
      <td className="py-2 pr-1 pl-2.5">
        ...
        >
          <span className="w-3 text-center font-mono text-[11px] text-muted-foreground">
            {pos}
          </span>
          ...
          >
            {team.name}
            {elim && (
              <span className="sr-only"> {t("groups.srEliminated")}</span>
            )}
          </span>
          {isClinched(d) && d.symbol && (
            <span
              title={
                d.kind === "wonGroup"
                  ? t("groups.wonGroupTitle")
                  : t("groups.advancedTitle")
              }
              className={
                d.kind === "wonGroup"
                  ? "text-[10px]"
                  : "text-[9px] font-bold text-win"
              }
            >
              {d.symbol}
  ...
  return (
    <div className="mt-6 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-muted-foreground">
      <span className="flex items-center gap-1.5">
        <span className="inline-block h-3 w-1 rounded-sm bg-win" />{" "}
        {t("groups.legendDirect")}
      </span>
      <span className="flex items-center gap-1.5">
        <span className="inline-block h-3 w-1 rounded-sm bg-contention" />{" "}
        {t("groups.legendThird")}
      </span>
      <span className="flex items-center gap-1.5">
        <span className="font-bold text-win">✓</span>{" "}
        {t("groups.legendClinched")}
      </span>
      <span className="flex items-center gap-1.5">
        <span className="line-through">{t("groups.legendTeamWord")}</span>{" "}
        {t("common.eliminated")}
      </span>
