...
const teamsRec: Record<string, TeamProb> = Object.fromEntries(
  p.teams.map((t) => [t.code, t])
)
const t0 = performance.now()
const awards = await computeAwards(p.matches, teamsRec, getMatchSummary)
console.log(
  `computeAwards: ${(performance.now() - t0).toFixed(0)}ms · aggregated ${awards.matchesCounted} matches`
)
...
console.log(
  "  G  A  player                  team   proj  win%   (mp, koMatchesLeft)"
)
