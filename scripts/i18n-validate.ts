...
for (const loc of LOCALES) {
  ...
  const status =
    missing.length || extra.length ? "❌" : phDrift.length ? "⚠️ " : "✅"
  ...
  console.log(
    `${status} ${loc}: ${lk.size} keys` +
      (missing.length
        ...
        : "") +
      (extra.length
        ? ` | EXTRA ${extra.length}: ${extra.slice(0, 6).join(", ")}${extra.length > 6 ? "…" : ""}`
        : "") +
      (phDrift.length
        ? ` | placeholder drift ${phDrift.length}: ${phDrift.slice(0, 4).join("; ")}${phDrift.length > 4 ? "…" : ""}`
        : "")
  )
}
