  ...
  for (let a = 0; a < 7; a++) {
    try {
      const r = await fetch(url, {
        headers: { "User-Agent": "WC2026Predictions/1.0 (asset mirror)" },
      })
      if (r.ok)
        return {
...
async function venues() {
  const { VENUE_PHOTOS } = (await import("@/lib/data/venuePhotos")) as {
    VENUE_PHOTOS: Record<
      string,
      {
        url: string
  ...
  for (const [slug, p] of Object.entries(VENUE_PHOTOS)) {
    ...
    if (p.url.includes("blob.vercel-storage.com")) {
    ...
    const cardUrl = card
      ? await upload(`venues/${slug}-card.${ext}`, card.buf, card.ct)
      : heroUrl
  ...
  const withSlug = Object.fromEntries(
    Object.entries(out).map(([s, v]) => [s, { slug: s, ...v }])
  )
  writeFileSync(
    "lib/data/venuePhotos.ts",
    hdr + JSON.stringify(withSlug, null, 0) + ";\n"
  )
...
async function players() {
  ...
  for (const [key, url] of entries) {
    ...
    if (!url) {
      out[key] = ""
      continue
    } // confirmed no photo — record so getPlayerImage short-circuits
    ...
    if (!img) {
      console.log(`  ${key}: download failed (will retry next run)`)
      continue
    } // not recorded → retried
    out[key] = await upload(
      `players/${code.toLowerCase()}-${slugify(name)}.png`,
      img.buf,
      img.ct
    )
    if (++uploaded % 25 === 0) {
      writeOut()
      console.log(
        `  ...${uploaded} uploaded (${scanned}/${entries.length} scanned)`
      )
    }
    await sleep(120)
  }
...
if (which === "venues") await venues()
  ...
  console.log(
    "usage: bun run scripts/upload-assets-to-blob.ts <venues|players>"
  )
