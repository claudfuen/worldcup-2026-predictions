...
if (existsSync(FRAG_PATH)) {
  ...
  for (const f of frags) {
    const messages =
      isObj(f) && isObj((f as Json).messages)
        ? ((f as Json).messages as Json)
        : (f as Json)
    if (isObj(messages)) {
...
console.log(
  `Merged ${fragCount} fragment(s). en.json now has ${countLeaves(en)} strings across ${Object.keys(en).length} namespaces: ${Object.keys(en).sort().join(", ")}`
)
