...
const fixes: { locale: string; key: string; value: string }[] = JSON.parse(
  readFileSync(FIXES, "utf8")
)
...
for (const [locale, list] of byLocale) {
  ...
  for (const f of list) {
    if (!hasKey(en, f.key)) {
      console.warn(
        `  ⚠️  ${locale}: "${f.key}" not in en.json — skipping (orphan)`
      )
      orphan++
      continue
    }
    if (deepSet(data, f.key, f.value)) {
      ...
      console.warn(
        `  ⚠️  ${locale}: could not set "${f.key}" (missing path) — skipping`
      )
      skipped++
    }
  }
