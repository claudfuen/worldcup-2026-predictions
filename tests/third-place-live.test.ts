...
function groupWith(
  letter: string,
  third: { pts: number; gd: number; gf: number }
): GroupView {
  const code = `T${letter}`
...
const GROUPS: GroupView[] = LETTERS.map((l, i) =>
  groupWith(l, { pts: 12 - i, gd: 0, gf: 0 })
)
const ratings: Ratings = Object.fromEntries(
  GROUPS.flatMap((g) => g.teams.map((t) => [t.code, 1500]))
)
...
describe("liveThirdPlaceRace — no live matches (pure cron-standings reconstruction)", () => {
  ...
  it("marks exactly the top 8 as advancing", () => {
    expect(race.filter((e) => e.advancing).map((e) => e.group)).toEqual([
      "A",
      "B",
      "C",
      "D",
  ...
  it("ranks are 1..12 in order", () => {
    expect(race.map((e) => e.rank)).toEqual([
...
describe("liveThirdPlaceRace — applies the 2026 cross-group tiebreakers (pts -> GD -> GF)", () => {
  it("breaks equal points by goal difference, then goals for", () => {
    ...
    const r: Ratings = Object.fromEntries(
      gs.flatMap((g) => g.teams.map((t) => [t.code, 1500]))
    )
    const prov: Record<string, ProvisionalGroup | null> = {
      A: null,
      B: null,
      C: null,
    }
...
describe("liveThirdPlaceRace — a live (in-progress) group contributes its PROVISIONAL third", () => {
  it("uses prov.rows[2] for live groups and re-ranks the cut accordingly", () => {
    ...
    const provByGroup: Record<string, ProvisionalGroup | null> =
      Object.fromEntries(LETTERS.map((l) => [l, l === "I" ? prov : null]))
    const race = liveThirdPlaceRace(GROUPS, provByGroup, ratings)
