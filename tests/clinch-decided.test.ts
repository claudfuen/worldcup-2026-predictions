...
function achievablePositions(
  matches: GroupMatch[]
): Record<string, Set<number>> {
  const pos: Record<string, Set<number>> = Object.fromEntries(
    CODES.map((c) => [c, new Set<number>()])
  )
...
function assertRowInvariants(cl: TeamClinch) {
  ...
  if (cl.second) {
    expect(cl.top2).toBe(true)
    expect(cl.winner).toBe(false)
  }
  if (cl.top2) {
    expect(cl.eliminatedTop2).toBe(false)
    expect(cl.guaranteedTop3).toBe(true)
  }
  if (cl.eliminatedTop2) {
    expect(cl.top2).toBe(false)
    expect(cl.winner).toBe(false)
    expect(cl.second).toBe(false)
  }
  if (cl.eliminatedTop3) {
    expect(cl.eliminatedTop2).toBe(true)
    expect(cl.guaranteedTop3).toBe(false)
  }
  ...
  it("3rd/4th split by GD: 3rd is a guaranteed top-3, 4th is fully eliminated", () => {
    ...
    for (const code of CODES) {
      for (const f of FIELDS)
        expect(c[code][f], `${code}.${f}`).toBe(oracleClinch(oracle[code])[f])
    }
  })
})
    ...
    for (let n = 0; n < 4000; n++) {
      ...
      for (const code of CODES) {
        ...
        for (const f of FIELDS) {
          expect(
            c[code][f],
            `iter ${n} ${code}.${f}\n${JSON.stringify(m.map((x) => [x.home, x.away, x.homeGoals, x.awayGoals]))}`
          ).toBe(exp[f])
        }
      }
