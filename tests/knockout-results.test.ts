...
describe("resolveKnockoutResults", () => {
  it("decides a penalty shootout by the advancing flag, not the (drawn) score", () => {
    ...
    const { winners, losers, played } = resolveKnockoutResults(
      groupOutcome,
      slotToTeam,
      res
    )
    ...
    expect(played[73]).toMatchObject({
      home,
      away,
      homeScore: 1,
      awayScore: 1,
  ...
  it("orients the score to the bracket's home/away even if ESPN lists them the other way", () => {
    ...
    expect(played[73]).toMatchObject({
      ...
      awayScore: 2,
      winner: away,
    }) // re-oriented to bracket home(=home)
  })

  it("propagates a played R32 winner into its R16 feeder, and leaves unplayed matches absent", () => {
    const r16 = KNOCKOUT.find(
      (m) =>
        m.round === "R16" && (m.home.startsWith("W") || m.away.startsWith("W"))
    )!
    const feederNo = Number(
      ((m) => (m.home.startsWith("W") ? m.home : m.away))(r16).slice(1)
    )
    ...
    const { winners, played } = resolveKnockoutResults(
      groupOutcome,
      slotToTeam,
      res
    )
