...
const T = [...groupCodes].sort(
  (a, b) => (ratings[a] as number) - (ratings[b] as number)
)[0]
...
function winGroup(live: LiveMatch[]): number {
  return runMonteCarlo(buildGroupMatches([], live), ratings, ITER, SEED).teams[
    T
  ].winGroup
}
...
describe("live-conditioned Monte Carlo", () => {
  ...
  it("a live KNOCKOUT match conditions its advancement (and propagates downstream)", () => {
    const r32matches = KNOCKOUT.filter((m) => m.round === "R32").map(
      (m) => m.match
    )
    ...
    const homeWinRate = (liveKO: KOLive) => {
      ...
      for (let i = 0; i < 2500; i++)
        if (simulateKnockout(r32, ratings, rand, {}, liveKO).winners[mn] === H)
          c++
      return c / 2500
    }
    const base = homeWinRate({})
    const leading = homeWinRate({
      [[H, A].sort().join("-")]: {
        homeCode: H,
        homeScore: 3,
        awayScore: 0,
        frac: fracRemaining(88),
      },
    })
  ...
  it("a live match with an unknown clock is frozen at its current score (counted as played)", () => {
    ...
    const fixture = built[GROUP].find(
      (m) =>
        (m.home === T && m.away === OPP) || (m.home === OPP && m.away === T)
    )!
