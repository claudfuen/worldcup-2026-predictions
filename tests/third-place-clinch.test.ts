...
const codesOf = (g: string) =>
  TEAMS.filter((t) => t.group === g).map((t) => t.code)
...
const statusOfThird = (gm: Record<string, GroupMatch[]>) => {
  const { groups } = buildGroupViews(gm, ratings, () => ({
    winGroup: 0,
    advance: 0,
  }))
...
describe("buildGroupViews — best-third clinch respects settled GD/GF tiebreakers", () => {
  ...
  it("leaves still-playing groups and the lower decided thirds as live (no false clinch)", () => {
    for (const g of ["E", "F", "G", "H", "I", "J", "K", "L"])
      expect(status[g]).toBe("live")
  })

  it("never marks more than 8 thirds as through (a top-8 invariant)", () => {
    const through = GROUPS.filter((g) =>
      ["won_group", "second", "advanced"].includes(status[g])
    )
...
describe("buildGroupViews — best-third ELIMINATION respects settled GD/GF tiebreakers", () => {
  ...
  const T3_GDm5 = [
    ...
    [1, 0],
    [3, 0],
  ] as [number, number][] // 3rd: 3 pts, GD -5
  const T3_GD0 = [
    [0, 0],
    [2, 0],
    [1, 0],
    ...
    [1, 0],
    [1, 0],
  ] as [number, number][] //  3rd: 3 pts, GD  0
  const T1 = [
    [0, 0],
    [1, 1],
    [1, 0],
    ...
    [1, 0],
    [1, 0],
  ] as [number, number][] //      3rd: 1 pt (below)

  const gm: Record<string, GroupMatch[]> = {}
  gm["A"] = mk("A", T3_GDm5) // target
  for (const g of ["B", "C", "D", "E", "F", "G", "H", "I"])
    gm[g] = mk(g, T3_GD0) // 8 thirds tied on pts, better GD
