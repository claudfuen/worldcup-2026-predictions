...
function groupMatches(
  ...
  for (const s of GROUP_SCHED) {
    ...
    byGroup[s.group!].push({
      ...
      away: s.away!,
      played: fin,
      homeGoals: fin ? h : undefined,
      awayGoals: fin ? a : undefined,
    })
  }
  return byGroup
}
...
function matchInfos(
  statusOf: (m: number) => "final" | "live" | "scheduled"
): MatchInfo[] {
  return GROUP_SCHED.map((s) => {
    ...
    return {
      ...
      defined: true,
      status: st,
      homeScore: scored ? h : undefined,
      awayScore: scored ? a : undefined,
    } as MatchInfo
  })
}
...
function sig(
  groups: {
    group: string
    decided: boolean
    teams: { code: string; pts: number; gd: number; status: string }[]
  }[]
) {
  return groups.map((g) => ({
    group: g.group,
...
describe("finalizeGroups — idempotent when nothing new beyond the cron payload", () => {
  ...
  it("partial (some finals) with overlay matching cron: no change", () => {
    ...
    const cron = buildGroupViews(
      groupMatches(isFinal),
      ratings,
      dummyProb
    ).groups
...
describe("finalizeGroups — a match that just went final finalizes instantly", () => {
  ...
  const groupA = GROUP_SCHED.filter((s) => s.group === "A").sort((a, b) =>
    a.utc.localeCompare(b.utc)
  )
  const lastA = groupA[groupA.length - 1].match

  const cronGroups = buildGroupViews(
    groupMatches((m) => m !== lastA),
    ratings,
    dummyProb
  ).groups
  ...
  it("finalized Group A matches a fresh full-build of the same results", () => {
    const fresh = buildGroupViews(
      groupMatches(() => true),
      ratings,
      dummyProb
    ).groups.find((g) => g.group === "A")!
...
describe("finalizeGroups — in-progress matches move the STANDINGS but never the clinch/decided state", () => {
  ...
  it("Group A is NOT decided (a live game is not full-time) and no team clinches off the live score", () => {
    ...
    const statusByCode = (gv: typeof A) =>
      Object.fromEntries(gv.teams.map((t) => [t.code, t.status]))
  ...
  it("but the live score IS reflected in goal difference / points / order", () => {
    ...
    const asIfPlayed = buildGroupViews(
      groupMatches(() => true),
      ratings,
      dummyProb
    ).groups.find((g) => g.group === "A")!
    const standings = (gv: typeof A) =>
      gv.teams.map((t) => `${t.code}:${t.pts}:${t.gd}`)
