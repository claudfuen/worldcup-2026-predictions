...
const goal = (
  ...
  player: string,
  assist?: string,
  goalType: "goal" | "penalty" | "own" = "goal"
): MatchEvent => ({
  kind: "goal",
  minute: "10'",
  sortMinute: 10,
...
const mkMatch = (
  ...
  home: string,
  away: string,
  status: MatchInfo["status"] = "final",
  round = "GROUP"
): MatchInfo => ({ match, home, away, status, round }) as MatchInfo
...
const getSummary = async (m: MatchInfo): Promise<MatchSummary> =>
  summaries[m.match] ?? { events: [], stats: null }
const matches = [
  mkMatch(1, "AAA", "BBB"),
  mkMatch(2, "AAA", "CCC", "scheduled"),
]
...
describe("computeAwards", () => {
  it("ranks the Golden Boot by goals and is deterministic for a seed", async () => {
    ...
    expect(a.goldenBoot.map((e) => [e.player, e.winProb])).toEqual(
      b.goldenBoot.map((e) => [e.player, e.winProb])
    )
  })
...
describe("computeAwards elimination", () => {
  it("marks a player whose team is out of matches and behind the leader as eliminated", async () => {
    ...
    const gs = async (m: MatchInfo): Promise<MatchSummary> =>
      sums[m.match] ?? { events: [], stats: null }
    const ms = [
      mkMatch(1, "AAA", "BBB"),
      mkMatch(10, "ZZZ", "P1"),
      mkMatch(11, "ZZZ", "P2"),
    ...
    const tms: Record<string, TeamProb> = {
      AAA: team({
        code: "AAA",
        advance: 0.9,
        r16: 0.7,
  ...
  it("clinches the award only once every match is final (tournament over)", async () => {
    ...
    const sums: Record<number, MatchSummary> = {
      1: {
        events: [goal("AAA", "Ava"), goal("AAA", "Ava"), goal("AAA", "Ava")],
        stats: null,
      }, // Ava 3
  ...
  it("does NOT clinch while any match is still to play", async () => {
    ...
    const tms: Record<string, TeamProb> = {
      AAA: team({ code: "AAA", advance: 0.9, r16: 0.6, qf: 0.4, sf: 0.2 }),
    }
    const { goldenBoot } = await computeAwards(ms, tms, gs, 1)
