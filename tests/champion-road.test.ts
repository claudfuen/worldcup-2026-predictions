...
const cand = (code: string, prob: number): SlotCandidate => ({
  code,
  name: code,
  prob,
})
...
const tree = () => [
  ko(101, "SF", {
    slotHome: "W97",
    slotAway: "W98",
    projHome: [cand("ESP", 0.6), cand("MAR", 0.4)],
    projAway: [cand("FRA", 0.7), cand("GER", 0.3)],
  }),
  ko(102, "SF", {
    slotHome: "W99",
    slotAway: "W100",
    projHome: [cand("ARG", 0.55), cand("BRA", 0.45)],
    projAway: [cand("ENG", 0.5), cand("POR", 0.5)],
  }),
  ko(97, "QF", {
    slotHome: "W89",
    slotAway: "W90",
    projHome: [cand("MAR", 0.5), cand("ESP", 0.45)],
    projAway: [cand("USA", 0.6), cand("JPN", 0.4)],
  }),
  ko(104, "FINAL", {
    slotHome: "W101",
    slotAway: "W102",
...
describe("applyChampionRoad", () => {
  ...
  it("walks the W## feeder chain down the champion's side", () => {
    ...
    expect(
      matches.find((m) => m.match === 101)!.projHome!.map((c) => c.code)
    ).toEqual(["ESP", "MAR"])
    expect(
      matches.find((m) => m.match === 97)!.projHome!.map((c) => c.code)
    ).toEqual(["ESP", "MAR"])
  ...
  it("follows the bottom half when the champion's reach prob is higher there", () => {
    ...
    expect(
      matches.find((m) => m.match === 102)!.projHome!.map((c) => c.code)
    ).toEqual(["BRA", "ARG"])
  })
  ...
  it("passes through a slot already resolved to the champion and keeps descending", () => {
    ...
    const final = matches.find((m) => m.match === 104)!
    final.home = "ESP"
    final.homeName = "Spain"
    final.projHome = undefined // SF played: ESP is IN the final
    applyChampionRoad(matches, "ESP")
