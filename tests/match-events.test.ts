...
const goal = (
  ...
  participants: [
    ...
    ...(assist ? [{ athlete: { displayName: assist } }] : []),
  ],
})
...
describe("parseKeyEvents", () => {
  it("parses goals (scorer + assist), penalties, own goals, and cards; sorts by minute", () => {
    ...
    expect(evs.map((e) => e.minute)).toEqual([
      "30'",
      "60'",
      "67'",
      "75'",
    ...
    expect(evs.find((e) => e.player === "Some Striker")!.goalType).toBe(
      "penalty"
    )
    ...
    const evs = parseKeyEvents([
      {
        ...
        scoringPlay: false,
        participants: [
          { athlete: { displayName: "Djed Spence" } },
          { athlete: { displayName: "Jarell Quansah" } },
        ],
      },
    ] as never)
