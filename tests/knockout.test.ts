...
describe("buildR32 resolves slot references correctly", () => {
  ...
  it("M75 = 1F vs 2C (distinct from M76)", () =>
    expect(r32[75]).toEqual(["Fw", "Cr"]))
  it("M73 = 2A vs 2B", () => expect(r32[73]).toEqual(["Ar", "Br"]))
  it("M84 = 1H vs 2J", () => expect(r32[84]).toEqual(["Hw", "Jr"]))
  it("M79 = 1A (Mexico) vs its assigned third", () =>
    expect(r32[79]).toEqual(["Aw", "X1"]))
  it("M82 = 1G vs its assigned third", () =>
    expect(r32[82]).toEqual(["Gw", "X5"]))
...
describe("simulateKnockout", () => {
  ...
  it("a far-stronger field member wins more often", () => {
    ...
    const rs = Object.fromEntries(
      Object.values(r32)
        .flat()
        .map((c) => [c, c === strong ? 3000 : 1400])
    )
    let wins = 0
    for (let s = 0; s < 200; s++)
      if (simulateKnockout(r32, rs, mulberry32(s)).champion === strong) wins++
