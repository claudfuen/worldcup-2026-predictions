...
const m = (over: Partial<MatchInfo>): MatchInfo =>
  ({
    status: "final",
    round: "GROUP",
    home: "AAA",
...
describe("computeMatchAccuracy", () => {
  ...
  it("returns null with no completed matches", () => {
    expect(
      computeMatchAccuracy([m({ status: "scheduled", probs: undefined })])
    ).toBeNull()
  })
...
describe("computeTournamentAccuracy", () => {
  const pre = {
    ARG: { advance: 0.95, title: 0.3 },
    BRA: { advance: 0.9, title: 0.25 },
    FRA: { advance: 0.85, title: 0.2 },
  }
  ...
  it("leaves advance hit-rate unset until all 32 R32 teams are known", () => {
    const out = computeTournamentAccuracy(
      [m({ round: "R32", home: "ARG", away: "BRA" })],
      undefined,
      pre
    )
