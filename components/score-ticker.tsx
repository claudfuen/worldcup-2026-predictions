...
export function ScoreTicker({
  ...
  return (
    <div className="z-40 overflow-hidden border-b border-border/60 bg-background/70 backdrop-blur-xl md:sticky md:top-14">
      <div
...
function TickerItem({
  ...
  return (
    <Link
      ...
      className="flex min-h-[40px] shrink-0 items-center gap-1.5 border-r border-border/40 px-4 py-2.5 text-xs whitespace-nowrap hover:bg-muted/30 sm:min-h-0 sm:py-1.5"
    >
      {live && (
        <span
      ...
      {live || final ? (
        ...
        <span className="px-0.5 text-muted-2">{t("scoreTicker.v")}</span>
      )}
      {ps && (
        <span
          className="font-mono text-[10px] text-muted-2 tabular-nums"
      ...
      <span
        className={`ml-0.5 font-mono text-[10px] font-semibold tracking-wide uppercase ${live ? "text-live" : final ? "text-win" : "text-data-cool"}`}
      ...
      >
        {live ? (
          ...
          <>
            {dayWord} <LocalTime utc={m.utc} mode="timeshort" />
          </>
        ) : (
