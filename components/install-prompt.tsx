...
export function InstallPrompt() {
    ...
    try {
      if (!sessionStorage.getItem(SESSION_GUARD)) {
        localStorage.setItem(
          SESSIONS_KEY,
          String(Number(localStorage.getItem(SESSIONS_KEY) ?? "0") + 1)
        )
  ...
  async function install() {
    if (deferred) {
      ...
      trackEvent("pwa_install_instructions", {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      <button
        ...
        tabIndex={-1}
        onClick={dismiss}
        className="absolute inset-0 animate-in bg-black/60 backdrop-blur-sm duration-200 fade-in"
      />
      <div
        ref={dialogRef}
        role="dialog"
        ...
        tabIndex={-1}
        className="relative w-full max-w-sm animate-in rounded-3xl border border-border-strong bg-surface-raised p-6 text-center shadow-2xl duration-200 outline-none zoom-in-95 fade-in slide-in-from-bottom-2 dark:shadow-none dark:inset-ring dark:inset-ring-white/5"
      >
        <button
          type="button"
          onClick={dismiss}
          aria-label={t("install.close")}
          className="absolute top-3 right-3 flex size-8 items-center justify-center rounded-lg text-muted-2 hover:bg-muted/40 hover:text-foreground"
        >
          <svg
            viewBox="0 0 24 24"
            width="16"
        ...
        {!iosTip ? (
          <>
            <span
              className="mx-auto flex size-16 items-center justify-center rounded-2xl border border-primary/30 bg-primary/10"
              aria-hidden
            >
              <svg
                viewBox="0 0 24 24"
            ...
            <h2 className="mt-4 font-display text-lg font-semibold tracking-tight text-balance">
              {t("install.title", { brand: t("meta.siteName") })}
            </h2>
            ...
            <button
              type="button"
              onClick={dismiss}
              className="mt-1 w-full rounded-lg px-4 py-2 text-sm text-muted-foreground hover:text-foreground"
            >
          ...
          <>
            ...
            {isIos() ? (
              <p className="mt-1.5 text-sm text-pretty text-muted-foreground">
                ...
                {t("install.iosAfter")}{" "}
                <span className="font-medium text-foreground">
                  {t("install.iosMenuItem")}
                </span>
              </p>
            ) : (
              <p className="mt-1.5 text-sm text-pretty text-muted-foreground">
                {t("install.androidBefore")}
                <span className="font-medium text-foreground">⋮</span>
                {t("install.androidMid")}{" "}
                <span className="font-medium text-foreground">
                  {t("install.androidMenuItem")}
                </span>{" "}
                {t("install.androidOr")}{" "}
                <span className="font-medium text-foreground">
                  {t("install.androidInstallApp")}
                </span>
              </p>
            )}
            <button
              type="button"
              onClick={dismiss}
              className="mt-5 w-full rounded-lg px-4 py-2 text-sm text-muted-foreground hover:text-foreground"
            >
              {t("install.gotIt")}
            </button>
          </>
