...
function pickMatch(matches: MatchInfo[]): MatchInfo | null {
  const withVenue = matches.filter(
    (m) => m.venue && VENUE_PHOTOS[VENUE_BY_KEY[m.venue]?.slug ?? ""]
  )
  const live = withVenue
  ...
  const upcoming = withVenue
    .filter(
      (m) => m.status === "scheduled" && Date.parse(m.utc) >= now - 30 * 60_000
    )
    .sort((a, b) => a.utc.localeCompare(b.utc))
  if (upcoming.length) return upcoming[0]
  const done = withVenue
...
export async function StadiumSpotlight({
  matches,
  className = "",
}: {
  matches: MatchInfo[]
  ...
  return (
    <section
      className={`group relative overflow-hidden rounded-2xl border border-border dark:inset-ring dark:inset-ring-white/5 ${className}`}
    >
      ...
      />
      <div
        className="absolute inset-0 bg-gradient-to-t from-[rgba(6,10,8,0.92)] via-[rgba(6,10,8,0.45)] to-[rgba(6,10,8,0.55)]"
        aria-hidden
      />
      <div className="relative flex min-h-[260px] flex-col justify-between gap-4 p-5 text-white sm:min-h-[300px] sm:p-6">
        <div className="flex items-center gap-2">
          <span
            className={`inline-flex items-center gap-1.5 rounded-md px-2 py-0.5 font-mono text-[10px] font-semibold tracking-[0.12em] uppercase ${m.status === "live" ? "bg-live/90 text-white" : "bg-white/15 text-white"}`}
          >
            {m.status === "live" && (
          ...
          <span
            className="font-mono text-[11px] tracking-wide text-white/75"
        ...
        <div>
          <Link
            ...
            className="block"
          >
            <div className="text-2xl font-semibold tracking-tight text-balance sm:text-3xl">
              {v.fifaName}
            </div>
            <div className="mt-1 inline-flex items-center gap-1.5 text-sm text-white/80">
              <Flag code={v.hostCode} size={16} /> {v.city},{" "}
              {t(`teams.${v.hostCode}`)}
            </div>
          </Link>
          ...
          >
            <span className="inline-flex items-center gap-1.5">
              <Flag code={h.code} size={18} /> {h.name ?? t("common.tbd")}
            </span>
            <span className="text-white/55">{t("common.vs")}</span>
            <span className="inline-flex items-center gap-1.5">
              <Flag code={a.code} size={18} /> {a.name ?? t("common.tbd")}
            </span>
          </Link>
        </div>
      </div>
      <div className="absolute right-2 bottom-1.5 font-mono text-[9px] text-white/45">
        {photo.artist}
