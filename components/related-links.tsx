...
export function RelatedLinks({ title = "Keep exploring", links }: { title?: string; links: RelLink[] }) {
  const items = links.filter(Boolean);
  if (items.length === 0) return null;
  return (
    <section className="border-border/60 mt-10 border-t pt-5">
      <h2 className="text-muted-foreground mb-3 font-mono text-xs font-semibold tracking-wide uppercase">{title}</h2>
