...
const ACCENT: Record<
  Accent,
  { text: string; solid: string; ghost: string; glow: string }
> = {
  gold: {
    text: "text-contention",
    solid: "bg-contention",
    ghost: "bg-contention/25",
    glow: "from-contention/[0.08]",
  },
  cool: {
    text: "text-data-cool",
    solid: "bg-data-cool",
    ghost: "bg-data-cool/25",
    glow: "from-data-cool/[0.08]",
  },
}
...
function ProjBar({
  ...
  const gain =
    max > 0
      ? Math.min(100 - cur, (Math.max(0, projected - current) / max) * 100)
      : 0
  return (
    <div
      className={`flex overflow-hidden rounded-full bg-muted/40 dark:inset-ring dark:inset-ring-white/5 ${className}`}
      aria-hidden
    >
...
export async function AwardsBoard({
  entries,
  metric,
  accent,
  limit = 20,
  ...
  if (rows.length === 0)
    return (
      <p className="rounded-2xl border border-border bg-card px-4 py-6 text-center text-sm text-muted-2 dark:inset-ring dark:inset-ring-white/5">
        {t("awards.boardEmpty")}
      </p>
    )
  const max = Math.max(...rows.map((e) => e.projected), 1)
  const [leader, ...rest] = rows
  const leaderImg = await getPlayerImage(leader.player, leader.teamCode).catch(
    () => null
  )
  return (
    <div>
      ...
      />
      {rest.length > 0 && (
        <div className="mt-3 rounded-2xl border border-border bg-card p-1.5 dark:inset-ring dark:inset-ring-white/5">
          <ol start={2} className="list-none divide-y divide-border/50">
            {rest.map((e, i) => (
              <Row
...
function FeaturedLeader({
  ...
  const unit =
    metric === "goals" ? t("awards.goalsAbbr") : t("awards.assistsAbbr")
  const left = Math.round(entry.matchesLeft)
  return (
    <Link
      href={localeHref(
        locale,
        `/player/${playerSlug(entry.player, entry.teamCode)}`
      )}
      className="group relative block overflow-hidden rounded-2xl border border-border bg-card p-4 transition-colors sm:p-5 dark:inset-ring dark:inset-ring-white/5"
    >
      <div
        className={`pointer-events-none absolute inset-0 bg-gradient-to-br ${a.glow} to-transparent to-40%`}
        aria-hidden
      />
      <div className="relative flex items-start gap-3 sm:gap-4">
        ...
        <div className="min-w-0 flex-1">
          <div
            className={`font-mono text-[10px] font-semibold tracking-[0.12em] uppercase ${a.text}`}
          >
            {t("awards.leaderLabel")}
          </div>
          <div className="mt-0.5 truncate font-display text-xl font-semibold tracking-tight group-hover:text-foreground/90 sm:text-2xl">
            {entry.player}
          </div>
        ...
        <div className="shrink-0 text-right">
          {entry.clinched ? (
            <div
              className={`inline-flex items-center gap-1.5 font-mono text-xl font-semibold tracking-[0.08em] uppercase sm:text-2xl ${a.text}`}
            >
              <svg
                viewBox="0 0 24 24"
                width="18"
            ...
            <>
              <div
                className={`font-mono text-3xl leading-none font-semibold tabular-nums sm:text-4xl ${a.text}`}
              >
                {forecastPct(entry.winProb)}
              </div>
              <div className="mt-1 font-mono text-[10px] tracking-wide text-muted-2 uppercase">
                {t("awards.toWin")}
              </div>
            </>
          )}
      ...
      <div className="relative mt-4">
        <div className="mb-1.5 flex items-baseline justify-between text-xs">
          <span>
            <span className="font-mono text-[10px] tracking-wide text-muted-2 uppercase">
              {t("awards.now")}{" "}
            </span>
          ...
          <span>
            ...
            <span
              className={`font-mono text-base font-semibold tabular-nums ${a.text}`}
            >
...
function Row({
  ...
  const value = valueOf(entry, metric)
  const unit =
    metric === "goals" ? t("awards.colGoals") : t("awards.colAssists")
  return (
    <li>
      <Link
        href={localeHref(
        ...
        className={`flex items-center gap-2.5 rounded-md px-1.5 py-2.5 transition-colors hover:bg-muted/20 ${entry.eliminated ? "opacity-55" : ""}`}
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="truncate text-xs text-muted-2">
            {t(`teams.${entry.teamCode}`)}
            {metric === "goals" && entry.penalties > 0 && (
              <span> · {t("awards.penNote", { n: entry.penalties })}</span>
            )}
          </div>
        </div>
        {/* current tally */}
        ...
        {entry.eliminated ? (
          ...
          <div
            className="flex w-[5.5rem] shrink-0 justify-end sm:w-32"
            title={t("awards.outTitle")}
          >
          ...
          <div className="flex w-[5.5rem] shrink-0 items-center gap-1.5 sm:w-32">
            ...
            />
            <span className="w-7 shrink-0 text-right font-mono text-[11px] text-muted-2 tabular-nums">
              {entry.projected.toFixed(1)}
              <span className="sr-only"> {t("awards.projected")}</span>
            </span>
          </div>
        )}
