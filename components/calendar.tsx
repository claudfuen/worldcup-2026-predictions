...
export function Calendar({ matches }: { matches: MatchInfo[] }) {
  ...
  useEffect(
    () => setTodayKey(fmtDayKey(new Date().toISOString(), zone)),
    [zone]
  )
  ...
  const weekdays = Array.from(
    { length: 7 },
    (_, i) =>
      new Intl.DateTimeFormat(loc, { weekday: "short" }).format(
        new Date(2024, 0, 1 + i)
      ) // Mon-start
  )
  const shortDate = (d: Date) =>
    new Intl.DateTimeFormat(loc, { month: "short", day: "numeric" }).format(d)
  ...
  for (let w = new Date(start); ; w.setDate(w.getDate() + 7)) {
    ...
    if (last >= firstKey)
      weeks.push({ days: days.map((d) => new Date(d)), first, last })
    if (weeks.length > 12) break // safety
  }
  const hasPast = mounted && weeks.some((wk) => wk.last < todayKey!)
  const futureWeeks =
    mounted && !showPast ? weeks.filter((wk) => wk.last >= todayKey!) : weeks
  ...
  const futureKeys =
    mounted && !showPast ? allKeys.filter((k) => k >= todayKey!) : allKeys
  ...
  const showEarlier = hasPast && !showPast && (
    <button
      ...
      className="mb-4 inline-flex items-center gap-1.5 rounded-full border border-border/70 px-3 py-1.5 text-xs text-muted-foreground hover:border-border hover:text-foreground"
    >
      <svg
        viewBox="0 0 24 24"
        width="13"
  ...
  return (
    <div suppressHydrationWarning>
      ...
      <div className="z-20 -mx-4 mb-4 border-b border-border/40 bg-background/90 px-4 py-2.5 backdrop-blur-md sm:-mx-6 sm:px-6 md:sticky md:top-14 lg:-mx-8 lg:px-8">
          ...
          {PHASES.filter((p) => present.has(p.key)).map((p) => (
            ...
            >
              <span
                ...
                style={{ backgroundColor: mix(p.color, 55) }}
                aria-hidden
              />
        ...
        {visibleWeeks.map((wk) => (
          <div key={wk.first}>
            <div className="mb-1.5 font-mono text-[11px] text-muted-2">
              {shortDate(wk.days[0])} – {shortDate(wk.days[6])}
            </div>
            {/* all-day phase bars */}
            <div className="mb-1 grid grid-cols-7 gap-1.5">
              {runsFor(wk.days).map((run) => (
                <div
                  key={run.start}
                  style={{
                    gridColumn: `${run.start + 1} / span ${run.len}`,
                    backgroundColor: mix(PHASE_COLOR[run.phase], 16),
                    color: PHASE_COLOR[run.phase],
                  }}
            ...
            <div className="grid grid-cols-7 gap-1.5">
              {wk.days.map((d) => {
                ...
                if (!inRange)
                  return <div key={key} className="min-h-24 rounded-lg" />
                ...
                return (
                  <div
                    key={key}
                    className={`min-h-24 rounded-lg border p-1 ${isToday ? "border-primary ring-2 ring-primary/30" : "border-border/40"}`}
                    style={
                      color
                        ? { backgroundColor: mix(color, isToday ? 9 : 5) }
                        : undefined
                    }
                  >
                    <div className="mb-1 flex items-center gap-1 px-0.5">
                      ...
                      <span
                        className={`ml-auto font-mono text-[11px] ${isToday ? "flex h-[18px] min-w-[18px] items-center justify-center rounded-full bg-primary px-1 font-bold text-background" : "text-muted-foreground"}`}
                      >
                        {d.getDate()}
                      </span>
                    </div>
        ...
        {visibleKeys.map((key) => {
          ...
          return (
            <section
              ...
              className={
                isToday
                  ? "-mx-2 rounded-2xl border border-primary/40 bg-primary/[0.05] px-3 py-3"
                  : ""
              }
            >
              <h2
                className="mb-2 flex items-center gap-2 font-mono text-xs font-semibold tracking-wide uppercase"
                suppressHydrationWarning
              >
                <span
                  className="h-2 w-3.5 shrink-0 rounded-sm"
                  style={{ backgroundColor: mix(color, 55) }}
                  aria-hidden
                />
                <span className="text-muted-foreground">
                  {fmtDay(items[0].utc, zone)}
                </span>
                <span className="normal-case" style={{ color }}>
                  · {t(`rounds.${phase}`)}
                </span>
                {isToday && (
                  <span className="rounded bg-primary/10 px-1.5 py-0.5 text-[10px] tracking-normal text-primary normal-case">
                    {t("calendar.today")}
                  </span>
                )}
              </h2>
...
function MatchCard({
  ...
  const homeName =
    m.homeName ?? m.projHome?.[0]?.name ?? m.slotHome ?? t("common.tbd")
  const awayName =
    m.awayName ?? m.projAway?.[0]?.name ?? m.slotAway ?? t("common.tbd")
  const homeWin =
    final &&
    (m.winner ? m.winner === m.home : (m.homeScore ?? 0) > (m.awayScore ?? 0))
  const awayWin =
    final &&
    (m.winner ? m.winner === m.away : (m.awayScore ?? 0) > (m.homeScore ?? 0))
  const ps = final && decidedOnPens(m) ? pensScore(m) : null
  return (
    <Link
      href={localeHref(locale, `/match/${m.match}`)}
      className="block rounded-lg border border-border/70 bg-card transition-colors hover:border-primary/40 hover:bg-muted/30"
    >
      <div className="flex items-center justify-between gap-1 px-2 pt-1.5 text-[10px] text-muted-2">
        <span className="truncate font-mono" suppressHydrationWarning>
          {live ? (
            ...
            <span className="font-semibold text-win">{t("home.ft")}</span>
          ) : (
            fmtTimeShort(m.utc, zone)
          )}
        </span>
        <span className="shrink-0 truncate">
          {m.group ? `${m.group} · ` : ""}M{m.match}
        </span>
      </div>
      <div className="px-2 pt-1 pb-1">
        <Side
          ...
          code={homeCode}
          name={homeName}
          score={final || live ? m.homeScore : undefined}
          pen={ps?.home}
          win={homeWin}
          cands={m.projHome}
          knockout={m.round !== "GROUP"}
        />
        ...
        <Side
          ...
          code={awayCode}
          name={awayName}
          score={final || live ? m.awayScore : undefined}
          pen={ps?.away}
          win={awayWin}
          cands={m.projAway}
          knockout={m.round !== "GROUP"}
      ...
      <div className="truncate border-t border-border/40 px-2 py-1 text-[10px] text-muted-2">
        {fifaVenue(m.venue)}
      </div>
    </Link>
  )
...
function Side({
  ...
  if (resolved && knockout) {
    return (
      <div className="flex items-center gap-1.5 px-0.5 py-px">
        ...
        {score != null && (
          <span
            className={`shrink-0 font-mono text-[12px] tabular-nums ${win ? "font-bold text-foreground" : "text-muted-foreground"}`}
          >
            {score}
            {pen != null && (
              <span className="ms-0.5 text-[10px] font-normal text-muted-2">
                ({pen})
              </span>
            )}
          </span>
          ...
          return (
            ...
            >
              <span
                ...
                style={{
                  width: `${w}%`,
                  backgroundColor: mix("var(--foreground)", lead ? 13 : 5),
                }}
                aria-hidden
              />
              <span className="relative flex w-full items-center gap-1.5 px-0.5 py-px">
                ...
                <span
                  className={`min-w-0 flex-1 truncate ${lead ? "text-[13px] font-semibold text-foreground" : i === 1 ? "text-[11px] text-muted-foreground" : "text-[11px] text-muted-2"}`}
                >
                  {c.name}
                </span>
                <span
                  className={`shrink-0 font-mono tabular-nums ${lead ? "text-[12px] font-semibold text-foreground/90" : "text-[10px] text-muted-2"}`}
                >
                  {pct(Math.min(c.prob, 0.99))}
                </span>
              </span>
            </div>
          )
  ...
  return (
    <TeamLine
      ...
      win={win}
      projected={!resolved}
      prob={!resolved ? cands?.[0]?.prob : undefined}
    />
  )
}
...
function TeamLine({
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span
        className={`min-w-0 flex-1 truncate text-xs ${win ? "font-semibold" : projected ? "text-foreground/70" : ""}`}
      >
        {name}
        {projected && prob != null && (
      ...
      {score != null && (
        <span
          className={`shrink-0 font-mono text-xs tabular-nums ${win ? "font-bold text-foreground" : "text-muted-foreground"}`}
        >
          {score}
        </span>
      )}
