...
export async function AdvanceBadge({
  ...
  if (d.kind === "forecast") {
    ...
    if (showDelta) {
      return (
        <span
          className={`inline-flex items-center justify-end font-mono text-xs font-semibold tabular-nums ${TONE_CLASS[d.tone]}`}
        >
    ...
    return (
      <span
        className={`font-mono text-xs font-semibold whitespace-nowrap tabular-nums ${TONE_CLASS[d.tone]}`}
      >
        {d.pct}
      </span>
    )
  ...
  return (
    <span
      className={`font-semibold whitespace-nowrap ${variant === "full" ? "font-mono text-xs" : ""} ${TONE_CLASS[d.tone]}`}
    >
      {text}
