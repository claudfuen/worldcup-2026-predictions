...
function survival(e: ThirdPlaceEntry, all: ThirdPlaceEntry[]) {
  const above = (x: ThirdPlaceEntry) =>
    x.pts !== e.pts ? x.pts > e.pts : x.gd !== e.gd ? x.gd > e.gd : x.gf > e.gf
  ...
  const dangers = [...swing]
    ...
    .slice(0, 2)
    .map((x) => `${x.name} ${forecastPct(x.advanceProb)}`)
  return {
    needBelow,
...
function RowTip({
  ...
  const through =
    e.status === "won_group" || e.status === "second" || e.status === "advanced"
  ...
  if (e.status === "eliminated") {
    ...
    if (s.needBelow <= 0) {
      ...
      headline = t("groups.tipHolding", {
        ordinal: ordinal(e.rank, intl),
        pct,
        frac,
        letters: s.letters,
      })
      ...
      const dangers = s.dangers.length
        ? t("groups.tipDangers", { dangers: s.dangers.join(", ") })
        : ""
      headline = t("groups.tipBelowCut", {
        ordinal: ordinal(e.rank, intl),
        pct,
        frac,
        letters: s.letters,
  ...
  const showOpps =
    e.status !== "eliminated" && !e.opponent && (e.opponents?.length ?? 0) > 0

  return (
    <>
      <p className="leading-snug text-foreground/90">{headline}</p>
      {showOpps && (
        <div className="mt-2.5">
          <div className="mb-1 font-mono text-[9px] font-semibold tracking-wide text-muted-2 uppercase">
...
function Row({
  ...
  return (
    <tr
      {...tip.triggerProps}
      {...tip.tapProps}
      className={`border-s-2 ${e.advancing ? "border-s-contention" : "border-s-transparent"} ${e.rank === 8 ? "border-b border-dashed border-b-primary/50" : ""} ${elim ? "opacity-45" : ""} hover:bg-muted/20`}
    >
      <td className="cursor-help py-2 ps-3 pe-1 font-mono text-[11px] text-muted-foreground">
        {e.rank}
      </td>
      <td className="py-2">
        <div className="flex min-w-0 items-center gap-2">
          <Link
            href={localeHref(locale, `/team/${slugForCode(e.code)}`)}
            className="flex min-w-0 items-center gap-2 hover:underline"
          >
            <Flag code={e.code} size={20} />
            <span
              className={`min-w-0 flex-1 truncate text-[13px] font-medium ${elim ? "line-through" : ""}`}
            >
              {e.name}
            </span>
          </Link>
          <Link
            href={localeHref(locale, `/group/${e.group.toLowerCase()}`)}
            className="shrink-0 text-[11px] text-muted-foreground hover:text-primary hover:underline"
          >
            {t("groups.grpShort", { group: e.group })}
          </Link>
        </div>
      </td>
      <td className="cursor-help px-1 text-center font-mono text-xs text-muted-foreground tabular-nums">
        {e.gf}
      </td>
      <td className="cursor-help px-1 text-center font-mono text-xs tabular-nums">
        {e.gd >= 0 ? "+" : ""}
        {e.gd}
      </td>
      <td className="cursor-help px-1 text-center font-mono text-[13px] font-bold tabular-nums">
        {e.pts}
      </td>
      <td className="cursor-help px-2 pe-3 text-right text-xs tabular-nums">
        {through ? (
          <span
            ...
            title={
              decided ? t("groups.thirdSetTitle") : t("groups.thirdInTitle")
            }
          >
          ...
          <ProbMeter
            p={e.advanceProb}
            className={`justify-end ${e.advancing ? "text-contention" : "text-muted-foreground"}`}
          />
        )}
      </td>
      {tip.open && (
        <HoverTipPanel pos={tip.pos}>
          <RowTip e={e} all={all} t={t} intl={localeConfig(locale).intl} />
        </HoverTipPanel>
      )}
    </tr>
  )
    ...
    <section className="mt-10">
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border/60 text-[11px] tracking-wide text-muted-foreground">
              <th className="py-2 ps-3 pe-1 text-left font-medium">
                {t("groups.thirdColRank")}
              </th>
              <th className="py-2 text-left font-medium">
                {t("groups.thirdColTeam")}
              </th>
              <th className="w-8 px-1 text-center font-medium">
                {t("groups.colGf")}
              </th>
              <th className="w-8 px-1 text-center font-medium">
                {t("groups.colGd")}
              </th>
              <th className="w-8 px-1 text-center font-semibold">
                {t("groups.colPts")}
              </th>
              <th
                className="px-2 pe-3 text-right font-medium"
                title={t("groups.thirdColChanceTitle")}
              >
                {t("groups.thirdColChance")}
              </th>
            </tr>
          </thead>
          <tbody>
            {entries.map((e) => (
              <Row key={e.code} e={e} all={entries} t={t} locale={locale} />
            ))}
          </tbody>
        </table>
      </div>
