...
export function CommandMenu() {
  ...
  const openMenu = useCallback(() => {
    ...
    if (!fetchedRef.current) {
      ...
      fetch("/api/search-index")
        .then((r) => r.json())
        .then((d) => {
          setMatchData(d.matches ?? [])
          setPlayerData(d.players ?? [])
          if (d.suggest) setSuggest(d.suggest)
        })
        .catch(() => {
  ...
  const items = useMemo<Item[]>(() => {
    ...
    for (const p of PAGES) {
      ...
      out.push({
        ...
        label,
        href: p.href,
        keywords: `${label} ${PAGE_ALIAS[p.href] ?? ""}`.toLowerCase(),
      })
    }
    for (const tm of TEAMS) {
      ...
      out.push({
        ...
        type: "team",
        label: name,
        sub: t("cmd.group", { letter: tm.group }),
        href: `/team/${slugForCode(tm.code)}`,
        code: tm.code,
        keywords:
          `${name} ${tm.code} group ${tm.group} ${TEAM_ALIAS[tm.name] ?? ""}`.toLowerCase(),
      })
    }
    ...
    for (const p of playerData) {
      ...
      out.push({
        ...
        type: "player",
        label: p.name,
        sub: posLabel ? `${team} · ${posLabel}` : team,
        href: `/player/${p.slug}`,
        code: p.team,
        keywords:
          `${p.name} ${team} ${p.team} ${posLabel} ${POS_KW[p.pos ?? ""] ?? ""}`.toLowerCase(),
      })
    }
    const fmtShort = new Intl.DateTimeFormat(localeConfig(locale).intl, {
      month: "short",
      day: "numeric",
    })
    ...
    if (matchData) {
      ...
      for (const s of SCHEDULE) {
        ...
        const label =
          s.home && s.away
            ? `${nm(s.home)} ${vs} ${nm(s.away)}`
            : `${roundLabel} · ${t("cmd.matchN", { n: s.match })}`
  ...
  const sections = useMemo<
    { key: string; header: string | null; items: Item[] }[]
  >(() => {
    ...
    if (q) {
      ...
      const tokenMatch = (i: Item, tok: string) =>
        i.keywords.includes(tok) ||
        (tok.length >= 4 &&
          (i.words ?? []).some((w) => w.length >= 3 && lev1(tok, w)))
      const scored = items
    ...
    const pick = (ids: string[]) =>
      ids.map((id) => byId.get(id)).filter((x): x is Item => !!x)
    ...
    if (live.length)
      out.push({
        key: "live",
        header: t("cmd.secLive"),
        items: pick(live.map((m) => `match:${m.n}`)),
      })
    const teams = pick(suggest.teams.map((c) => `team:${c}`))
    if (teams.length)
      out.push({ key: "teams", header: t("cmd.secFavorites"), items: teams })
    const scorers = pick(suggest.players.map((s) => `player:${s}`))
    if (scorers.length)
      out.push({ key: "scorers", header: t("cmd.secScorers"), items: scorers })
    out.push({
      key: "pages",
      header: t("cmd.secPages"),
      items: pick(
        ["/bracket", "/schedule", "/venues", "/awards"].map((h) => `page:${h}`)
      ),
    })
  ...
  return (
    ...
    >
      ...
      />
      <div
        className="relative w-full max-w-xl overflow-hidden rounded-2xl border border-border-strong bg-surface-raised shadow-2xl dark:inset-ring dark:inset-ring-white/10"
        onKeyDown={onKeyDown}
      >
        <div className="flex items-center gap-2.5 border-b border-border/70 px-4">
          ...
          <input
            ...
            className="h-14 w-full bg-transparent text-base text-foreground outline-none placeholder:text-muted-2"
            role="combobox"
            aria-expanded
            aria-controls="cmd-results"
            aria-autocomplete="list"
          />
          <kbd className="hidden shrink-0 rounded border border-border bg-muted/40 px-1.5 py-0.5 font-mono text-[10px] text-muted-2 sm:block">
            esc
          </kbd>
        </div>
        <div
            ...
            (() => {
              ...
              return sections.map((sec) => (
                <div key={sec.key} role="group">
                  {sec.header && (
                    <div className="px-4 pt-2.5 pb-1 font-mono text-[10px] font-semibold tracking-wide text-muted-2 uppercase">
                      {sec.header}
                    </div>
                  )}
                  {sec.items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="shrink-0 font-mono text-[10px] tracking-wide text-muted-2 uppercase">
                          {t(`cmd.type_${item.type}`)}
                        </span>
                      </button>
                    )
                  })}
        ...
        <div className="hidden items-center gap-4 border-t border-border/70 px-4 py-2 text-[10px] text-muted-2 sm:flex">
          <span>
            <Kbd>↑</Kbd>
            <Kbd>↓</Kbd> {t("cmd.hintNav")}
          </span>
          <span>
            <Kbd>↵</Kbd> {t("cmd.hintOpen")}
          </span>
          <span className="ms-auto">
            <Kbd>esc</Kbd> {t("cmd.hintClose")}
          </span>
        </div>
      </div>
    </div>
...
const POS_KW: Record<string, string> = {
  ...
  DF: "defender back",
  MF: "midfielder",
  FW: "forward striker attacker winger",
}

function Kbd({ children }: { children: React.ReactNode }) {
  return (
    <kbd className="mr-0.5 inline-block rounded border border-border bg-muted/40 px-1 font-mono text-[10px]">
      {children}
    </kbd>
  )
}
