...
function useMatchLive(
  matchNo: number,
  initial: MatchLivePayload
): MatchLivePayload {
...
function prettySlot(t: TFunction, s?: string): string {
  ...
  if (s.startsWith("3:"))
    return t("match.slotThird", { groups: s.slice(2).split(",").join("/") })
...
function shareTextFor(
  ...
  if (state === "final") {
    ...
    return decidedOnPens(m) && ps
      ? `${base} (${t("common.penScore", { home: ps.home, away: ps.away })})`
      : base
  }
  if (m.favorite && m.home && m.away)
    return t("match.shareFavorite", {
      favorite: nm(t, m.favorite.code) ?? m.favorite.name,
      pct: Math.round(m.favorite.winProb * 100),
      home: homeName,
      away: awayName,
    })
...
function AdvanceBar({
  ...
  if (secondary) {
    return (
      <div>
        <div className="flex h-1.5 w-full overflow-hidden rounded-full bg-muted/40 dark:inset-ring dark:inset-ring-white/5">
          <div className="bg-primary/55" style={{ width: `${home * 100}%` }} />
          <div
            className="bg-data-cool/55"
            style={{ width: `${away * 100}%` }}
          />
        </div>
        <div className="mt-1.5 flex items-baseline justify-between gap-2 text-xs text-muted-2">
          <span className="min-w-0 truncate">
            <span className="font-mono tabular-nums">{forecastPct(home)}</span>{" "}
            {homeName}
          </span>
          <span className="min-w-0 truncate text-right">
            {awayName}{" "}
            <span className="font-mono tabular-nums">{forecastPct(away)}</span>
          </span>
        </div>
      </div>
    )
  ...
  return (
    <div>
      <div className="flex h-2.5 w-full overflow-hidden rounded-full bg-muted/40 dark:inset-ring dark:inset-ring-white/5">
        <div
          className="bg-primary transition-[width] duration-700 ease-out"
          style={{ width: `${home * 100}%` }}
        />
        <div
          className="bg-data-cool transition-[width] duration-700 ease-out"
          style={{ width: `${away * 100}%` }}
        />
      </div>
      <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
        ...
        <div className="min-w-0 text-right">
          <div className="flex items-center justify-end gap-1.5">
            ...
            <span
              className="size-2 shrink-0 rounded-full bg-data-cool"
              aria-hidden
            />
          </div>
          <div className="mt-0.5 font-mono text-base font-semibold tabular-nums">
...
export function MatchHero({
  ...
  return (
    <section className="relative mt-5 overflow-hidden rounded-3xl border border-border-strong bg-surface-raised card-surface hero-sheen px-5 py-7 sm:px-10 sm:py-9 dark:inset-ring dark:inset-ring-white/8">
      <div className="mb-6 flex items-center justify-between gap-3">
        <span className="font-mono text-[11px] font-semibold tracking-[0.1em] text-primary uppercase">
          {m.group
            ? t("match.heroEyebrowGroup", {
                round: roundName(t, m.round),
                group: m.group,
              })
            : roundName(t, m.round)}{" "}
          <span className="text-muted-2">
            {t("match.heroMatchNo", { match: m.match })}
          </span>
        </span>
        <ShareBar
          text={shareTextFor(t, m, state, homeName, awayName)}
          path={`/match/${m.match}`}
          compact
        />
      </div>
      {state === "undefined" ? (
        <div className="flex items-start justify-center gap-3 sm:gap-12">
          ...
          <span
            className="flex size-11 shrink-0 items-center justify-center self-center rounded-full bg-background/40 font-mono text-[11px] font-semibold tracking-[0.12em] text-muted-2 uppercase ring-1 ring-white/5 ring-inset dark:bg-black/15"
            aria-hidden
          >
            {t("common.vs")}
          </span>
          ...
          <div className="flex shrink-0 flex-col items-center gap-2.5 rounded-xl bg-background/40 px-4 py-3 ring-1 ring-white/5 ring-inset dark:bg-black/15">
            {state === "final" || state === "live" ? (
              <span className="font-mono text-4xl font-semibold tracking-[-0.03em] tabular-nums sm:text-6xl">
                {m.homeScore}
                <span className="mx-2 align-middle text-[0.7em] font-normal text-muted-2">
                  –
                </span>
                {m.awayScore}
              </span>
            ) : (
              <span className="font-mono text-xs font-semibold tracking-[0.15em] text-muted-2 uppercase">
                {t("common.vs")}
              </span>
            )}
            {state === "final" ? (
              <span className="flex flex-col items-center gap-1">
                ...
                {onPens && (
                  <span
                    className="font-mono text-[11px] font-semibold tracking-[0.06em] text-win uppercase"
                    title={t("common.wonOnPenalties")}
                  >
                    {penLabel}
                  </span>
                )}
              ...
              <span
                className="font-mono text-[11px] font-semibold tracking-[0.1em] text-muted-2 uppercase"
            ...
            <>
              ...
              {m.probs && (
                <p className="mt-3 text-center text-xs text-pretty text-muted-2">
                  {t("match.inNinety", {
                    home: homeName,
                    homePct: forecastPct(m.probs.home),
                    drawPct: forecastPct(m.probs.draw),
                    away: awayName,
                    awayPct: forecastPct(m.probs.away),
                  })}
                  {m.probs.draw > 0 && (
                    <>
                      {" "}
      ...
      {state === "undefined" && (
        <p className="mx-auto mt-7 max-w-xl rounded-xl bg-background/30 px-5 py-4 text-center text-xs text-pretty text-muted-2 ring-1 ring-white/5 ring-inset dark:bg-black/10">
...
export function MatchFacts({
  ...
  const state = stateOf(m)
  return (
    <div className="mt-3 rounded-2xl border border-border bg-card/60 p-4 backdrop-blur-sm sm:p-5 dark:bg-card/50">
      <div className="grid grid-cols-2 gap-y-4 sm:grid-cols-4">
        <Fact
          label={t("match.factKickoff")}
          value={<LocalTime utc={m.utc} mode="datetime" />}
        />
        <Fact
          label={t("match.factVenue")}
          value={(() => {
            ...
            return slug ? (
              <>
                <Link
                  href={localeHref(locale, `/venues/${slug}`)}
                  className="underline-offset-2 hover:text-primary hover:underline"
                >
                  {venue}
                </Link>
                <span className="text-muted-2">, {m.city}</span>
              </>
            ) : (
        ...
        />
        <Fact
          label={t("match.factStage")}
          value={
            <>
              {m.round === "GROUP" && m.group ? (
                <Link
                  href={localeHref(locale, `/group/${m.group.toLowerCase()}`)}
                  className="underline-offset-2 hover:text-primary hover:underline"
                >
                  {t("match.groupLabel", { group: m.group })}
                </Link>
              ) : (
                <Link
              ...
              <span className="text-muted-2">
                {" "}
                {t("match.factMatchNo", { match: m.match })}
              </span>
            </>
          }
        />
        <Fact
          label={
            heat?.hot ? t("match.factWorthWatching") : t("match.factStatus")
          }
          value={
            heat?.hot ? (
              <span className="text-contention">
                {t(heat.reason.key, heat.reason.params)}
              </span>
            ) : state === "final" ? (
              t("match.fullTime")
            ) : state === "live" ? (
      ...
      {state !== "final" && state !== "live" && hasTickets(m.match) && (
        <div className="mt-4 flex flex-col gap-2.5 border-t border-border/50 pt-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-muted-2">
            {t("match.ticketsCaption", { provider: TICKET_PROVIDER })}
          </p>
          <TicketLink
            matchNo={m.match}
            placement="match_facts"
            variant="button"
...
function Fact({ label, value }: { label: string; value: React.ReactNode }) {
  ...
  return (
    <div className="min-w-0 border-border/60 [&:not(:nth-child(2n+1))]:border-l [&:not(:nth-child(2n+1))]:pl-4 sm:[&:not(:nth-child(4n+1))]:border-l sm:[&:not(:nth-child(4n+1))]:pl-6 sm:[&:nth-child(4n+1)]:border-l-0 sm:[&:nth-child(4n+1)]:pl-0">
      <div className="mb-1.5 font-mono text-[11px] font-semibold tracking-[0.1em] text-muted-2 uppercase">
        {label}
      </div>
      <div className="text-sm leading-snug text-foreground/90">{value}</div>
    </div>
  )
}
  ...
  const matchFacts = m.home && m.away && (
    <>
      <MatchTimeline
        ...
        scored={(m.homeScore ?? 0) + (m.awayScore ?? 0) > 0}
  ...
  return (
    <>
      {state === "defined" && m.probs && proseText && (
        <p className="mt-8 max-w-3xl text-sm text-pretty text-muted-foreground">
          {proseText}
        </p>
      )}
      ...
      {state === "final" && (
        <section className="mt-8">
          <h2 className="mb-3 font-mono text-xs font-semibold tracking-[0.1em] text-muted-foreground uppercase">
            {t("match.preMatchRead")}
          </h2>
          <div className="rounded-2xl border border-border bg-card card-surface p-4 dark:inset-ring dark:inset-ring-white/8">
            {m.probs ? (
              <>
                <WinProbBar
                  home={m.probs.home}
                ...
                <p className="mt-4 text-xs text-muted-2">
                  {m.xg && (
                    <>
                      {t("match.expectedGoalsPrefix", {
                        home: m.xg.home.toFixed(1),
                        away: m.xg.away.toFixed(1),
                      })}{" "}
                    </>
                  )}
                  {t("match.actualResult")}{" "}
      ...
      {state === "live" && m.probs && (
        <section className="mt-8">
          ...
          <div className="rounded-2xl border border-border bg-card card-surface p-4 dark:inset-ring dark:inset-ring-white/8">
            <p className="mb-3 text-xs font-medium text-live">
              {t("match.liveScoreLine", {
                home: homeName,
                homeScore: m.homeScore,
                awayScore: m.awayScore,
                away: awayName,
              ...
              <>
                ...
                {m.advance && (
                  <div className="mt-4 border-t border-border/50 pt-3">
                    <div className="mb-1.5 font-mono text-[10px] font-semibold tracking-wide text-muted-2 uppercase">
                      {t("match.preMatchLabel")}
                    </div>
                    <AdvanceBar
                      home={m.advance.home}
                      away={m.advance.away}
                ...
                <p className="mt-3 text-xs text-pretty text-muted-2">
                  {t("match.inNinety", {
                    home: homeName,
                    homePct: forecastPct(liveProbs.home),
                    drawPct: forecastPct(liveProbs.draw),
                    away: awayName,
                    awayPct: forecastPct(liveProbs.away),
                  })}
                </p>
              </>
            ) : liveProbs ? (
      ...
      {state === "defined" && m.topScores && m.topScores.length > 0 && (
        <section className="mt-8">
          ...
          <div className="divide-y divide-border/50 rounded-2xl border border-border bg-card card-surface dark:inset-ring dark:inset-ring-white/8">
            {m.topScores.map((s) => (
              <div
                key={`${s.h}-${s.a}`}
              ...
              >
                ...
                <span className="w-10 font-mono text-sm font-semibold tracking-[-0.01em] tabular-nums">
                  {s.h}–{s.a}
                </span>
                <Flag code={m.away} size={16} />
                <div className="relative ml-1 h-1.5 flex-1 overflow-hidden rounded-full bg-muted/40 dark:inset-ring dark:inset-ring-white/5">
                  <div
                    className="absolute inset-y-0 left-0 rounded-full bg-primary/85 transition-[width] duration-700 ease-out"
                    style={{
                      width: `${(s.prob / m.topScores![0].prob) * 100}%`,
                    }}
                  />
                </div>
                <span className="w-10 text-right font-mono text-xs font-semibold text-foreground/90 tabular-nums">
                  {pct(s.prob)}
                </span>
              </div>
            ))}
          ...
          <p className="mt-2 text-xs text-muted-2">
            {t("match.scorelinesNote", {
              ...
              rest: pct(
                Math.max(0, 1 - m.topScores.reduce((acc, s) => acc + s.prob, 0))
              ),
            })}
          </p>
        </section>
  ...
  if (!resolved || !name) {
    return (
      <div className="flex min-w-0 flex-1 flex-col items-center gap-3 text-center">
        ...
        <div className="text-sm text-muted-2">{t("common.tbd")}</div>
      </div>
    )
  }
  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col items-center gap-2 text-center">
      <Link
        ...
        className="group/team flex min-w-0 flex-col items-center gap-3 rounded-xl text-center transition-colors focus-visible:ring-2 focus-visible:ring-primary/50 focus-visible:ring-offset-2 focus-visible:ring-offset-surface-raised focus-visible:outline-none"
      >
        <span className={lose ? "opacity-60 saturate-[0.85]" : ""}>
          <Flag code={resolved} size={64} />
        </span>
        <div
          className={`font-display text-xl leading-tight font-semibold tracking-[-0.02em] text-balance decoration-primary/40 underline-offset-4 group-hover/team:text-primary group-hover/team:underline sm:text-2xl ${win ? "text-win" : lose ? "text-muted-foreground" : ""}`}
        >
          {name}
        </div>
      </Link>
      {chip && (eloRank != null || fifaRank != null) && (
        <button
          ...
          className="-m-1 cursor-pointer p-1 font-mono text-[10px] font-semibold tracking-[0.1em] text-muted-2 uppercase underline decoration-muted-foreground/40 decoration-dotted underline-offset-2 transition-colors hover:text-foreground"
        >
          {chip}
        </button>
      )}
  ...
  if (resolved && name) {
    return (
      <div className="flex min-w-0 flex-1 flex-col items-center gap-2.5 text-center">
        <div className="max-w-full truncate font-mono text-[11px] font-semibold tracking-[0.1em] text-muted-2 uppercase">
          {prettySlot(t, slot)}
        </div>
        <Link
          href={localeHref(locale, `/team/${slugForCode(resolved)}`)}
          className="group/team flex flex-col items-center gap-3 transition-colors"
        >
          <Flag code={resolved} size={64} />
          <div className="font-display text-xl leading-tight font-semibold tracking-[-0.02em] text-balance decoration-primary/40 underline-offset-4 group-hover/team:text-primary group-hover/team:underline sm:text-2xl">
            {name}
          </div>
        </Link>
        <div className="inline-flex items-center gap-1.5 font-mono text-[11px] font-semibold tracking-[0.1em] text-win uppercase">
          <svg
            viewBox="0 0 24 24"
            width="12"
            height="12"
  ...
  if (!top) {
    return (
      <div className="flex min-w-0 flex-1 flex-col items-center gap-3 text-center">
        ...
        <Flag code={null} size={64} />
        <div className="text-sm text-muted-2">{t("match.toBeDecided")}</div>
      </div>
    )
  }
  const rest = cands
    .slice(1)
    .filter((c) => c.prob >= 0.02)
    .slice(0, 3)
  return (
    <div className="flex min-w-0 flex-1 flex-col items-center gap-2.5 text-center">
      <div className="max-w-full truncate font-mono text-[11px] font-semibold tracking-[0.1em] text-muted-2 uppercase">
      ...
      <Link
        href={localeHref(locale, `/team/${slugForCode(top.code)}`)}
        className="group/team flex flex-col items-center gap-3 transition-colors"
      >
        <Flag code={top.code} size={64} />
        <div className="font-display text-xl leading-tight font-semibold tracking-[-0.02em] text-balance decoration-primary/40 underline-offset-4 group-hover/team:text-primary group-hover/team:underline sm:text-2xl">
          {nm(t, top.code) ?? top.name}
        </div>
      </Link>
      <div className="mt-0.5 w-full max-w-[13rem]">
        <div className="h-1.5 overflow-hidden rounded-full bg-muted/40 dark:inset-ring dark:inset-ring-white/5">
          <div
            className="h-full rounded-full bg-primary transition-[width] duration-700 ease-out"
            style={{ width: `${Math.round(Math.min(top.prob, 0.99) * 100)}%` }}
          />
        </div>
        <div className="mt-2.5">
          <div className="font-mono text-2xl font-semibold tracking-[-0.02em] text-primary tabular-nums sm:text-3xl">
            {forecastPct(top.prob)}
          </div>
          <div className="mt-0.5 font-mono text-[11px] font-semibold tracking-[0.1em] text-muted-2 uppercase">
            {t("match.toReachThisMatch")}
          </div>
        </div>
      </div>
      {rest.length > 0 && (
        <div className="mt-2 w-full max-w-[13rem] space-y-1.5 border-t border-border/50 pt-3">
          <div className="font-mono text-[11px] font-semibold tracking-[0.1em] text-muted-2 uppercase">
            {t("match.ifNot")}
          </div>
          {rest.map((r) => (
            <Link
