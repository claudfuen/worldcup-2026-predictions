...
export function ScheduleList({
  ...
  const yesterday = nowIso
    ? fmtDayKey(new Date(Date.parse(nowIso) - 86400000).toISOString(), zone)
    : null
  const tomorrow = nowIso
    ? fmtDayKey(new Date(Date.parse(nowIso) + 86400000).toISOString(), zone)
    : null
  ...
  const hasUpcoming =
    today == null ||
    matches.some((m) => fmtDayKey(m.utc, zone) >= (yesterday ?? today))
  ...
  const shown = matches.filter((m) => {
    ...
    if (today != null) {
      ...
      if (effectiveTime === "upcoming" && yesterday != null && day < yesterday)
        return false
  ...
  for (const m of [...shown].sort((a, b) => a.utc.localeCompare(b.utc))) {
    ...
    if (!d) {
      d = { key, label: dayLabel(key, fmtDay(m.utc, zone)), items: [] }
      days.push(d)
    }
    d.items.push(m)
  }
...
function Row({
  ...
  const homeLabel =
    ...
    (m.projHome?.[0]
      ? `${m.projHome[0].name}`
      : (m.slotHome ?? t("common.tbd")))
  const awayLabel =
    m.awayName ??
    (m.projAway?.[0]
      ? `${m.projAway[0].name}`
      : (m.slotAway ?? t("common.tbd")))
  ...
  return (
    <div className="relative flex items-center gap-3 px-3 py-2.5 transition-colors hover:bg-muted/30 sm:gap-4 sm:px-4">
      <Link
        href={localeHref(locale, `/match/${m.match}`)}
        className="absolute inset-0"
        aria-label={t("schedule.rowAria", { home: homeLabel, away: awayLabel })}
      />
      <div className="w-14 shrink-0 text-xs text-muted-foreground sm:w-16">
        <div className="font-mono whitespace-nowrap" suppressHydrationWarning>
          {fmtTimeShort(m.utc, zone)}
        </div>
        <div className="text-[10px] leading-tight">
          {ROUND_KEY[m.round] ? t(ROUND_KEY[m.round]) : m.round}
          {m.group ? ` ${m.group}` : ""}
        </div>
      </div>
      ...
      <div className="flex min-w-0 items-center gap-2.5">
        <div className="min-w-0 space-y-0.5">
          <TeamName
            ...
            win={homeWin}
            projected={!m.home}
            prob={!m.home ? m.projHome?.[0]?.prob : undefined}
          />
          <TeamName
            code={awayCode}
            label={awayLabel}
            win={awayWin}
            projected={!m.away}
            prob={!m.away ? m.projAway?.[0]?.prob : undefined}
          />
        </div>
        {showScore && (
      ...
      <div className="ml-auto flex shrink-0 items-center gap-2 sm:gap-2.5">
        ...
        <div className="text-right">
          {live ? (
            <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-live">
              ...
              {t("schedule.liveLabel")} {m.liveDetail}
            </span>
          ) : final ? (
            <span className="text-[11px] font-medium text-win">
              {t("schedule.fullTime")}
            </span>
          ) : m.favorite ? (
            <span className="text-[11px] text-muted-foreground">
              <span className="text-foreground/80">{m.favorite.name}</span>{" "}
              {pct(m.favorite.winProb)}
            </span>
          ) : (
            ...
            {(() => {
              ...
              return slug ? (
                <Link
                  ...
                  className="relative z-10 text-muted-2 underline-offset-2 hover:text-primary hover:underline"
                >
                  {fifaVenue(m.venue)}
                </Link>
              ) : (
        ...
        {upcoming && (
          <TicketLink
            matchNo={m.match}
            placement="schedule_row"
            variant="inline"
            className="relative z-10 -my-2 px-1 py-2"
          />
        )}
      </div>
    </div>
...
function TeamName({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span
        className={`max-w-[150px] truncate text-sm sm:max-w-[260px] ${win ? "font-semibold" : projected ? "text-foreground/75" : ""}`}
      >
        {label}
        {projected && prob != null && (
...
function Segmented({
  ...
  return (
    <div className="flex w-full shrink-0 rounded-lg border border-border bg-muted/30 p-0.5 sm:inline-flex sm:w-auto">
      {options.map((o) => (
        <button
          key={o.key}
          ...
          className={`flex-1 rounded-[0.3rem] px-3 py-2 text-sm whitespace-nowrap sm:flex-none sm:py-1 ${
            value === o.key
              ? "bg-surface-raised font-medium text-foreground inset-ring inset-ring-white/10"
