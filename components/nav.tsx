    ...
    <header className="sticky top-0 z-50 border-b border-border/70 bg-background/80 backdrop-blur-xl">
      <div className="mx-auto flex h-14 max-w-6xl items-stretch gap-1 px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden flex-1 items-stretch gap-1 md:flex">
          {PRIMARY.map((l) => {
            ...
            return (
              <Link
                ...
                className={`relative flex items-center px-3 text-sm whitespace-nowrap ${active ? "font-medium text-foreground" : "text-muted-foreground hover:text-foreground"}`}
              >
                {t(l.key)}
                {active && (
          ...
          <div className="relative flex items-stretch">
            <button
              ...
              aria-haspopup="menu"
              className={`relative flex items-center gap-1 px-3 text-sm whitespace-nowrap ${moreActive || moreOpen ? "font-medium text-foreground" : "text-muted-foreground hover:text-foreground"}`}
            >
              {t("nav.more")}
              <svg
                viewBox="0 0 24 24"
            ...
            {moreOpen && (
              <div
                role="menu"
                className="absolute top-full left-0 z-50 mt-1 min-w-44 rounded-xl border border-border bg-background/95 p-1 backdrop-blur-xl dark:inset-ring dark:inset-ring-white/10"
              >
                {SECONDARY.map((l) => {
                  const active = isActive(l.href)
                  return (
                    <Link
                      ...
                      className={`block rounded-lg px-3 py-2 text-sm ${active ? "bg-muted/50 font-medium text-foreground" : "text-muted-foreground hover:bg-muted/30 hover:text-foreground"}`}
                    >
                      {t(l.key)}
                    </Link>
                  )
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2 md:ml-3 md:gap-3">
          <button
            ...
            className="flex h-10 items-center gap-2 rounded-lg border-border/60 px-2.5 text-muted-foreground transition-colors hover:bg-muted/40 hover:text-foreground lg:border"
          >
            <svg
              viewBox="0 0 24 24"
              width="18"
            ...
            <kbd className="hidden shrink-0 rounded border border-border bg-muted/40 px-1.5 py-0.5 font-mono text-[10px] text-muted-2 lg:block">
              ⌘K
            </kbd>
          </button>
          <span
          ...
          <button
            ...
            aria-expanded={open}
            className="-mr-2 ml-1 flex size-10 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted/40 hover:text-foreground md:hidden"
          >
            {open ? (
              <svg
                viewBox="0 0 24 24"
      ...
      {open && (
        <>
          ...
          />
          <nav className="absolute inset-x-0 top-14 z-50 space-y-0.5 border-b border-border/70 bg-background/95 p-2 backdrop-blur-xl md:hidden dark:inset-ring dark:inset-ring-white/5">
            {ROUTES.map((l) => {
              const active = isActive(l.href)
              return (
                <Link
                  ...
                  href={localeHref(locale, l.href)}
                  onClick={() => setOpen(false)}
                  className={`flex items-center justify-between rounded-lg px-3 py-3 text-base ${active ? "bg-muted/50 font-medium text-foreground" : "text-muted-foreground hover:bg-muted/30 hover:text-foreground"}`}
                >
                  {t(l.key)}
                  {active && (
                    <span
            ...
            {!installed && (
              <button
                ...
                className="mt-1 flex w-full items-center gap-2 rounded-lg border-t border-border/60 px-3 py-3 text-base text-muted-foreground hover:bg-muted/30 hover:text-foreground"
              >
                <svg
                  viewBox="0 0 24 24"
                  width="18"
            ...
            <LanguageSwitcher
              variant="footer"
              className="mt-1 border-t border-border/60 px-3 pt-3 pb-1"
            />
          </nav>
        </>
      )}
...
function Freshness({ updatedAt }: { updatedAt: string | null }) {
  ...
  const min =
    now == null
      ? null
      : Math.max(0, (now - new Date(updatedAt).getTime()) / 60000)
  const rel =
    min == null
