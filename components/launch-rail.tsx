...
export async function LaunchRail({
  teams,
  iterations,
  className = "",
}: {
  ...
  const c1 = teams[0]
  return (
    <footer className={`border-t border-border/60 pt-6 ${className}`}>
      <nav className="flex flex-wrap items-center gap-x-5 font-mono text-xs font-medium tracking-wide uppercase">
        {LINKS.map((l) => (
          <Link
            ...
            className="inline-flex min-h-10 items-center text-muted-foreground hover:text-foreground"
          >
            {t(l.labelKey)}
          </Link>
        ))}
        ...
        <div className="mt-4">
          <ShareBar
            text={t("home.shareText", {
              team: c1.name,
              pct: forecastPct(c1.title),
              count: iterations.toLocaleString(intl),
            })}
            path="/"
          />
        </div>
      )}
      <p className="mt-5 text-xs text-pretty text-muted-2">
        {t("home.footerMethod")}{" "}
        <Link
      ...
      <p className="mt-1 text-xs text-muted-2">{t("home.footerDataNote")}</p>
    </footer>
  )
}
