    ...
    return (
      <div>
        <div className="flex h-1.5 w-full overflow-hidden rounded-full bg-muted/40 dark:inset-ring dark:inset-ring-white/5">
          ...
          />
          <div
            className="bg-data-cool/45"
            style={{ width: `${away * 100}%` }}
          />
        </div>
        <div className="mt-1.5 grid grid-cols-3 gap-2 font-mono text-xs text-muted-foreground tabular-nums">
  ...
  return (
    <div>
      <div className="flex h-2.5 w-full overflow-hidden rounded-full bg-muted/40 dark:inset-ring dark:inset-ring-white/5">
        ...
        />
        <div
          className="bg-muted-foreground/35 transition-[width] duration-700 ease-out"
          style={{ width: `${draw * 100}%` }}
        />
        <div
...
function Legend({
  ...
  return (
    <div className={`min-w-0 ${align}`}>
      <div
        className={`flex items-center gap-1.5 ${isRight ? "justify-end" : isCenter ? "justify-center" : ""}`}
      >
        {!isRight && (
