...
export function MatchFlagButton({
    ...
    if (!isAuthed) {
      const next =
        typeof window !== "undefined"
          ...
          : localeHref(locale, "/matches")
      router.push(
        `${localeHref(locale, "/signin")}?next=${encodeURIComponent(next)}`
      )
      return
    }
    const target = !on
    ...
    start(async () => {
      ...
      if (!res.ok) {
        ...
        if (res.error === "not-signed-in")
          router.push(localeHref(locale, "/signin"))
      } else {
        router.refresh()
      }
    })
  ...
  if (variant === "button") {
    return (
      ...
      >
        <Bookmark filled={on} />
        {on
          ? t("matchFlag.inMyMatches")
          : isAuthed
            ? t("matchFlag.addToMyMatches")
            : t("matchFlag.signInToSave")}
      </button>
    )
  }
  ...
  return (
    <button
      ...
      aria-pressed={on}
      aria-label={
        on ? t("matchFlag.removeFromMyMatches") : t("matchFlag.addToMyMatches")
      }
      title={
        on ? t("matchFlag.removeFromMyMatches") : t("matchFlag.addToMyMatches")
      }
