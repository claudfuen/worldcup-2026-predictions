...
function TeamLine({
  ...
  return (
    <div className="flex items-center gap-2 py-0.5">
      ...
      <span
        className={`min-w-0 flex-1 truncate text-sm font-medium ${dim ? "text-foreground/85" : ""}`}
      >
        {name ?? tbd}
      </span>
    </div>
...
export async function MatchesToWatch({
  ...
  return (
    <section className={className}>
      ...
      <p className="mb-3 text-xs text-pretty text-muted-2">
        {t("home.watchLede")}
      </p>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {plan.map((p) => (
          <WatchCard key={p.match.match} p={p} t={t} locale={locale} />
        ))}
      </div>
    </section>
  )
...
function WatchCard({
  ...
  const note = projected
    ? t("home.watchProjectedNote", {
        pct: Math.max(1, Math.round(p.lik * 100)),
        reason,
      })
    : reason
  const roundLabel =
  ...
  return (
    <Link
      ...
      className={`flex flex-col rounded-xl border bg-card p-4 transition-colors hover:border-primary/50 hover:bg-surface-raised ${projected ? "border-dashed border-border/70" : "border-border"}`}
    >
      <div className="mb-2.5 flex items-center justify-between gap-2 text-[11px] text-muted-foreground">
        <span className="min-w-0 truncate font-mono" suppressHydrationWarning>
          <LocalTime utc={m.utc} mode="day" /> ·{" "}
          <LocalTime utc={m.utc} mode="timeshort" />
        </span>
        <span
          className={`shrink-0 font-mono text-[10px] tracking-wide uppercase ${projected ? "text-primary/80" : "text-muted-2"}`}
        >
          {roundLabel}
        </span>
      </div>
