...
export function ShareBar({ text, path }: { text: string; path: string }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span className="text-muted-2 mr-1 text-[10px] font-semibold tracking-wide uppercase">Share</span>
      <a href={x} target="_blank" rel="noopener noreferrer" data-evt="share" data-channel="x" data-path={path} className={btn} aria-label="Share on X" title="Share on X">
      ...
      <a href={wa} target="_blank" rel="noopener noreferrer" data-evt="share" data-channel="whatsapp" data-path={path} className={btn} aria-label="Share on WhatsApp" title="Share on WhatsApp">
      ...
      <button type="button" onClick={copy} data-evt="share" data-channel="copy" data-path={path} className={`${btn} ${copied ? "text-win border-win/40" : ""}`} aria-label="Copy link" title={copied ? "Copied" : "Copy link"}>
        {copied ? (
