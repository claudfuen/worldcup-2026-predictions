...
export function ShareBar({ text, path }: { text: string; path: string }) {
  ...
  const copy = async () => {
    try {
      ...
      setCopied(true);
      trackEvent("share", { channel: "copy", path });
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <a href={x} target="_blank" rel="noopener noreferrer" onClick={() => trackEvent("share", { channel: "x", path })} className={btn} aria-label="Share on X" title="Share on X">
      ...
      <a href={wa} target="_blank" rel="noopener noreferrer" onClick={() => trackEvent("share", { channel: "whatsapp", path })} className={btn} aria-label="Share on WhatsApp" title="Share on WhatsApp">
      ...
      <button type="button" onClick={copy} className={`${btn} ${copied ? "text-win border-win/40" : ""}`} aria-label="Copy link" title={copied ? "Copied" : "Copy link"}>
        {copied ? (
