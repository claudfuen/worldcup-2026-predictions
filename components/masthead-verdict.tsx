...
export async function MastheadVerdict({
  ...
  const strip = teams.slice(0, close ? 4 : 5);
  ...
  return (
    <div>
      ...
      <div className="mt-3.5 flex flex-wrap items-center gap-x-5 gap-y-1.5">
        {strip.map((tm, i) => (
          <Link key={tm.code} href={localeHref(locale, `/team/${slugForCode(tm.code)}`)} className="group inline-flex items-baseline gap-1.5">
            <Flag code={tm.code} size={15} />
            <span className={`${i === 0 ? "text-foreground font-semibold" : "text-muted-foreground group-hover:text-foreground"}`}>{tm.name}</span>
            <span className={`font-mono text-sm tabular-nums ${i === 0 ? "text-primary font-semibold" : "text-muted-foreground"}`}>{forecastPct(tm.title)}</span>
