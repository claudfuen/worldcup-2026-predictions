...
const mix = (color: string, p: number) =>
  `color-mix(in oklab, ${color} ${p}%, transparent)`
  ...
  return (
    <>
      ...
      <div className="relative overflow-hidden rounded-2xl border border-border bg-card/20">
        <div className="[scrollbar-width:thin] overflow-x-auto overscroll-x-contain [mask-image:linear-gradient(to_right,transparent,#000_1.25rem,#000_calc(100%-1.25rem),transparent)] p-3 sm:p-4 md:[mask-image:none]">
          <div dir="ltr" className="flex min-w-[1200px] items-stretch">
            {ROUNDS.map((round, ri) => (
              <Fragment key={round}>
                <div className="flex min-w-[168px] flex-1 flex-col">
                  <div className="mb-3 h-4 px-1 font-mono text-[10px] font-semibold tracking-wide text-muted-foreground uppercase">
                    {t(`rounds.${round}`)}
                  </div>
                ...
                {ri < ROUNDS.length - 1 && (
                  <Connectors count={ORDER[ROUNDS[ri + 1]].length} />
                )}
              </Fragment>
            ))}
            {champion && (
              <>
                <div className="flex w-6 shrink-0 flex-col">
                  ...
                  <div className="flex flex-1 items-center">
                    <div className="w-full border-t border-muted-foreground/45" />
                  </div>
                </div>
      ...
      <p className="mt-2 text-center text-xs text-muted-2 md:hidden">
        {t("bracket.swipeHint")}
      </p>
    </>
  )
  ...
  return (
    <div
      ...
      className={`rounded-xl border p-3 text-center transition-[opacity,box-shadow] ${off ? "opacity-40" : ""} ${
        won
          ? "border-contention/45 bg-contention/[0.08]"
          : "border-primary/40 bg-primary/[0.07]"
      } ${on ? "ring-1 ring-primary/60" : ""}`}
    >
      <div
        className={`mb-1.5 font-mono text-[10px] font-semibold tracking-wide uppercase ${won ? "text-contention" : "text-primary"}`}
      >
        {won ? t("bracket.champion") : t("bracket.projectedChampion")}
      </div>
      <Link
        href={localeHref(locale, `/team/${slugForCode(champion.code)}`)}
      ...
      {won ? (
        <div className="mt-1 inline-flex items-center justify-center gap-1 font-mono text-[11px] font-semibold tracking-wide text-contention uppercase">
          <svg
            viewBox="0 0 24 24"
            width="12"
            height="12"
        ...
        <div className="mt-1 font-mono text-sm font-bold text-primary tabular-nums">
          {pct(Math.min(champion.prob, 0.99))}
        </div>
      )}
    </div>
  )
        ...
        {Array.from({ length: count }).map((_, i) => (
          <div key={i} className="flex flex-1 items-center">
            <div className="flex h-1/2 w-full items-center">
              <div className="h-full w-1/2 rounded-e-md border-e border-t border-b border-muted-foreground/45" />
              <div className="w-1/2 border-t border-muted-foreground/45" />
            </div>
          </div>
        ))}
      </div>
...
function Node({
  ...
  return (
    <Link
      ...
      className={`flex min-h-[124px] w-full flex-col rounded-xl border bg-card transition-[background-color,border-color,box-shadow,transform] hover:-translate-y-px hover:bg-muted/30 hover:shadow-lg hover:shadow-black/20 ${big ? "text-sm" : "text-xs"} ${
        nodeHit
          ? "border-primary/60 ring-1 ring-primary/25"
          : final
            ? "border-primary/45 bg-primary/[0.04] ring-1 ring-primary/15"
            : hasTicket
    ...
    >
      <div
        className={`flex items-center justify-between gap-1 ${final ? "text-primary/90" : "text-muted-foreground"} ${big ? "px-2.5 pt-2 pb-1 text-[10px]" : "px-2 pt-1.5 pb-1 text-[9px]"}`}
      >
        ...
        <span className="flex shrink-0 items-center gap-1.5">
          {m.status === "live" ? (
            <span className="inline-flex items-center gap-1 font-semibold text-live">
              <span className="size-1 animate-pulse rounded-full bg-live" />
              {m.liveDetail}
            </span>
          ) : m.status === "final" ? (
          ...
          {hasTicket && (
            <svg
              ...
              height="11"
              fill="currentColor"
              className="text-contention"
              aria-label={t("bracket.youHaveTickets")}
            >
...
function DecidedTie({
  ...
  big?: boolean
  championCode?: string
  zone: import("@/lib/format").Zone | undefined
  hovered: string | null
  onHover: (c: string | null) => void
  t: TFunction
}) {
  ...
  return (
    <div className={`flex flex-1 flex-col justify-center gap-1 py-1.5 ${px}`}>
      <TieTeam
        code={m.home!}
        name={m.homeName ?? m.home!}
        winProb={h}
        isChamp={m.home === championCode}
        big={big}
        hovered={hovered}
        onHover={onHover}
      />
      {h != null && a != null ? (
        <div className="my-0.5 flex h-1.5 gap-[2px]" aria-hidden>
          <span
            className="min-w-[3px] rounded-full bg-primary"
            style={{ width: `${h * 100}%` }}
          />
          <span
            className="min-w-[3px] rounded-full bg-data-cool"
            style={{ width: `${a * 100}%` }}
          />
        </div>
      ) : (
      ...
      <TieTeam
        code={m.away!}
        name={m.awayName ?? m.away!}
        winProb={a}
        isChamp={m.away === championCode}
        big={big}
        hovered={hovered}
        onHover={onHover}
      />
      ...
      >
        {kickoffPassed(m.utc) ? (
          ...
          <span className="font-mono text-[10px] font-semibold tracking-wide text-muted-2 uppercase">
            {t("bracket.awaitingResult")}
          </span>
        ) : (
          <>
            <svg
...
function TieTeam({
  ...
  return (
    ...
    >
      ...
      <span
        className={`min-w-0 flex-1 truncate ${big ? "text-sm" : "text-[13px]"} ${fav ? "font-semibold text-foreground" : "text-muted-foreground"}`}
      >
        {name}
      </span>
      {winProb != null && (
        <span
          className={`shrink-0 font-mono tabular-nums ${big ? "text-sm" : "text-xs"} ${on || (isChamp && hovered == null) ? "font-semibold text-primary" : fav ? "font-semibold text-foreground" : "text-muted-2"}`}
        >
          {pct(Math.min(winProb, 0.99))}
        </span>
      )}
    </div>
  )
...
function Side({
  ...
  if (resolved) {
    ...
    if (played || live) {
      ...
      return (
        ...
        >
          ...
          <span
            className={`min-w-0 flex-1 truncate ${big ? "text-sm" : "text-[13px]"} ${isLoser ? "text-muted-foreground line-through" : "font-semibold"}`}
          >
            {name}
          </span>
          {played ? (
            <span className="flex shrink-0 items-center gap-1">
              <span
                className={`font-mono font-bold tabular-nums ${big ? "text-sm" : "text-xs"} ${isWinner ? "text-win" : "text-muted-foreground"}`}
              >
                {score}
              </span>
              {onPens && ps != null && (
                <span
                  className={`font-mono text-[10px] tabular-nums ${isWinner ? "text-win" : "text-muted-foreground"}`}
              ...
              {onPens && ps == null && isWinner && (
                <span
                  className="font-mono text-[9px] font-semibold tracking-wide text-win/70 uppercase"
                  title={t("common.wonOnPenalties")}
                >
                  {t("common.pens")}
                </span>
            ...
            <span
              className={`shrink-0 font-mono font-bold text-foreground tabular-nums ${big ? "text-sm" : "text-xs"}`}
            >
              {score}
            </span>
          )}
    ...
    return (
      ...
      >
        <Flag code={resolved} size={big ? 24 : 20} />
        <span
          className={`min-w-0 flex-1 truncate font-semibold ${big ? "text-sm" : "text-[13px]"}`}
        >
          {name}
        </span>
      </div>
  ...
  if (list.length === 0) {
    return (
      <div
        className={`flex items-center ${big ? "gap-2.5 px-3 py-2.5" : "gap-2 px-2.5 py-2"}`}
      >
        <span
          className={`shrink-0 rounded-[3px] bg-muted/30 ${big ? "size-6" : "size-[18px]"}`}
          aria-hidden
        />
        <span className="text-xs text-muted-2">{t("common.tbd")}</span>
      </div>
    )
  }
  return (
...
function RaceRow({
  ...
  const fill = on
    ...
    : roadActive
      ? mix("var(--primary)", 22)
      : lead
        ? mix("var(--foreground)", 13)
        : mix("var(--foreground)", 5)
  return (
    <div
      onMouseEnter={() => onHover(code)}
      onMouseLeave={() => onHover(null)}
    ...
    >
      ...
      <span className="relative flex w-full items-center gap-1.5 px-1 py-0.5">
        ...
        {third && lead && (
          <span
            className="shrink-0 font-mono text-[8px] font-semibold tracking-wide text-muted-2 uppercase"
            title={t("bracket.thirdPlacedTeam")}
          >
            {t("rounds.shortThird")}
          </span>
        )}
        <span
          className={`min-w-0 flex-1 truncate ${lead ? "text-[13px] font-semibold text-foreground" : rank === 1 ? "text-[11px] text-muted-foreground" : "text-[11px] text-muted-2"}`}
        >
          {name}
        </span>
        <span
          className={`shrink-0 font-mono tabular-nums ${on || roadActive ? "text-[11px] font-semibold text-primary" : lead ? "text-[11px] font-semibold text-foreground/90" : "text-[10px] text-muted-2"}`}
        >
          {pct(Math.min(prob, 0.99))}
        </span>
      </span>
    </div>
  )
