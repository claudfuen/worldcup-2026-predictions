...
export function MatchStats({ stats }: { stats: MatchStatsData | null }) {
  ...
  const rows: {
    ...
    {
      label: t("match.statShotsOnTarget"),
      h: stats.home.shotsOnTarget,
      a: stats.away.shotsOnTarget,
    },
    {
      label: t("match.statCorners"),
      h: stats.home.corners,
      a: stats.away.corners,
    },
  ...
  return (
    <section className="mt-8">
      ...
      <div className="space-y-3.5 rounded-2xl border border-border bg-card p-4 dark:inset-ring dark:inset-ring-white/5">
        {rows.map((r) => (
...
function StatRow({
  ...
  const fmt = (v: number | null) =>
    v == null ? "–" : pctSuffix ? `${Math.round(v)}%` : `${v}`
  return (
    <div>
      <div className="mb-1 flex items-center justify-between gap-2 text-xs">
        <span className="w-10 font-mono font-semibold text-foreground tabular-nums">
          {fmt(h)}
        </span>
        <span className="truncate text-[10px] font-semibold tracking-wide text-muted-2 uppercase">
          {label}
        </span>
        <span className="w-10 text-right font-mono font-semibold text-foreground tabular-nums">
          {fmt(a)}
        </span>
      </div>
      <div className="flex h-1.5 overflow-hidden rounded-full bg-muted/40 dark:inset-ring dark:inset-ring-white/5">
        <div className="bg-primary/80" style={{ width: `${hShare}%` }} />
        <div
          className="bg-data-cool/70"
          style={{ width: `${100 - hShare}%` }}
        />
      </div>
    </div>
  )
