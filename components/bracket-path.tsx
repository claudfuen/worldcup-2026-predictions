...
function Side({
  ...
  return (
    <span
      className={`inline-flex min-w-0 items-center gap-1 ${proj ? "text-muted-foreground" : ""}`}
    >
...
function Matchup({ m, tbd, vs }: { m?: MatchInfo; tbd: string; vs: string }) {
  ...
  const home = m.homeName
    ? { code: m.home, name: m.homeName }
    : m.projHome?.[0]
      ? { code: m.projHome[0].code, name: m.projHome[0].name, proj: true }
      : { code: null, name: null }
  const away = m.awayName
    ? { code: m.away, name: m.awayName }
    : m.projAway?.[0]
      ? { code: m.projAway[0].code, name: m.projAway[0].name, proj: true }
      : { code: null, name: null }
  return (
    <span className="flex min-w-0 flex-1 items-center gap-1.5 truncate">
      <Side code={home.code} name={home.name} proj={home.proj} tbd={tbd} />
      <span className="text-muted-2">{vs}</span>
      <Side code={away.code} name={away.name} proj={away.proj} tbd={tbd} />
    </span>
  )
}
...
function PathRow({
  ...
  return (
    <Link
      href={href}
      className="flex items-center justify-between gap-3 px-4 py-2.5 text-sm hover:bg-muted/20"
    >
...
export async function BracketPath({
  m,
  all,
}: {
  m: MatchInfo
  ...
  const feeders = [m.slotHome, m.slotAway]
    .map((s) => {
      const g = s ? RX.exec(s) : null
      return g ? { kind: g[1] as "W" | "L", n: Number(g[2]) } : null
    })
    .filter((f): f is { kind: "W" | "L"; n: number } => f != null)
  const nextWin = all.find(
    (x) => x.slotHome === `W${m.match}` || x.slotAway === `W${m.match}`
  )
  const nextLose = all.find(
    (x) => x.slotHome === `L${m.match}` || x.slotAway === `L${m.match}`
  )
      ...
      <div className="divide-y divide-border/50 overflow-hidden rounded-2xl border border-border bg-card dark:inset-ring dark:inset-ring-white/5">
        {feeders.map((f) => (
          <PathRow
            ...
            rel={
              f.kind === "L"
                ? t("bracket.relLoserOf")
                : t("bracket.relWinnerOf")
            }
            m2={byNum.get(f.n)}
            tbd={tbd}
            vs={vs}
            href={localeHref(locale, `/match/${f.n}`)}
          />
        ))}
        {nextWin && (
          <PathRow
            to={nextWin.match}
            rel={t("bracket.relWinnerTo", {
              round: t(ROUND_KEY[nextWin.round]),
            })}
            m2={nextWin}
            tbd={tbd}
            vs={vs}
            href={localeHref(locale, `/match/${nextWin.match}`)}
          />
        )}
        {nextLose && (
          <PathRow
            to={nextLose.match}
            rel={t("bracket.relLoserTo", {
              round: t(ROUND_KEY[nextLose.round]),
            })}
            m2={nextLose}
            tbd={tbd}
            vs={vs}
            href={localeHref(locale, `/match/${nextLose.match}`)}
          />
        )}
      </div>
    </section>
