...
function opponent(m: MatchInfo, ourSlot: string) {
  ...
  return {
    ...
    name: name ?? proj?.name ?? null,
    locked: !!code,
    prob: code ? null : (proj?.prob ?? null),
  }
}
...
export async function R32ByFinish({
  matches,
  group,
  pred,
}: {
  ...
  const find = (slot: string) =>
    r32.find((m) => m.slotHome === slot || m.slotAway === slot)

  const rows = (
    [
      {
        key: "1",
        label: t("groups.routeWinGroup"),
        prob: pred.winGroup,
        slot: `1${group}`,
      },
      {
        key: "2",
        label: t("groups.routeRunnerUp"),
        prob: pred.runnerUp,
        slot: `2${group}`,
      },
      {
        key: "3",
        label: t("groups.routeBestThird"),
        prob: pred.third,
  ...
  return (
    <section className="mt-8">
        ...
        {rows.map((f) => {
          ...
          const inner = (
            <div className="flex items-center gap-3 px-4 py-3">
              <span
                className={`shrink-0 rounded-md border px-1.5 py-0.5 font-mono text-[10px] font-semibold tracking-wide uppercase ${TONE[f.key]}`}
              >
                {f.label}
              </span>
              ...
              {f.m ? (
                <>
                  <span
                    className="hidden shrink-0 text-xs text-muted-2 sm:inline"
                    suppressHydrationWarning
                  >
                    M{f.m.match} · <LocalTime utc={f.m.utc} mode="day" /> ·{" "}
                    {fifaCity(f.m.venue, f.m.city)}
                  </span>
                  <span className="flex min-w-0 flex-1 items-center justify-end gap-1.5 text-sm">
                    ...
                    {opp?.code ? (
                      <Flag code={opp.code} size={16} />
                    ) : (
                      <span
                        className="size-4 shrink-0 rounded-[2px] bg-muted/40"
                        aria-hidden
                      />
                    )}
                    <span
                      className={`truncate ${opp?.locked ? "font-semibold" : "text-foreground/80"}`}
                    >
                      {opp?.name ?? t("common.tbd")}
                    </span>
                    ...
                    {opp?.prob != null && (
                      <span className="hidden shrink-0 font-mono text-xs text-muted-foreground tabular-nums sm:inline">
                        {forecastPct(opp.prob)}
                      </span>
                    )}
                  </span>
                </>
              ) : (
                <span className="flex-1 text-right text-sm text-foreground/70">
                  {t("groups.routeFacesWinner")}{" "}
                  <span className="text-muted-2">
                    {t("groups.routeTieSet")}
                  </span>
                </span>
              )}
            </div>
          ...
          return f.m ? (
            <Link
              key={f.key}
              href={localeHref(locale, `/match/${f.m.match}`)}
      ...
      <p className="mt-2 text-xs text-pretty text-muted-2">
        {t("groups.routeFootnote", { name: pred.name, group })}
      </p>
    </section>
  )
}
