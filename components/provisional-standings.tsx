...
export function ProvisionalStandings({
  ...
  return (
    ...
    >
      <div className="space-y-1 border-b border-border/60 px-4 py-2.5">
        {proj.live.map((l) => (
          <div
            key={l.home + l.away}
          ...
          >
            ...
            <Link
              href={localeHref(
                locale,
                `/team/${teamSlug(TEAM_BY_CODE[l.home]?.name ?? l.home)}`
              )}
            ...
            <span className="shrink-0 font-mono font-bold tabular-nums">
              {l.homeGoals}&ndash;{l.awayGoals}
            </span>
            <Link
              href={localeHref(
                locale,
                `/team/${teamSlug(TEAM_BY_CODE[l.away]?.name ?? l.away)}`
              )}
              className="flex min-w-0 items-center gap-1.5 hover:underline"
            >
              <span className="min-w-0 truncate font-medium">
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {proj.rows.map((r, i) => {
            ...
            const isLive = proj.live.some(
              (l) => l.home === r.code || l.away === r.code
            )
            ...
            return (
              <tr
                key={r.code}
                className={`border-l-2 ${zone} ${out ? "opacity-45" : ""} ${isLive ? "bg-live/[0.06]" : ""}`}
              >
                <td className="py-2 pr-1 pl-2.5">
                  <Link
                    href={localeHref(
                      locale,
                      `/team/${teamSlug(TEAM_BY_CODE[r.code]?.name ?? r.code)}`
                    )}
                  ...
                  >
                    ...
                    <span
                      className={`truncate text-[13px] font-medium ${out ? "line-through" : ""}`}
                    >
                      {t(`teams.${r.code}`)}
                    </span>
                  </Link>
      ...
      <p className="border-t border-border/60 px-4 py-2.5 text-[11px] text-muted-2">
        {t("groups.provFootnoteLead")} <span className="text-win">✓</span>{" "}
        {t("groups.provFootnoteTail")}
      </p>
    </div>
  )
}
