...
function PlayerName({
  ...
  return (
    <Link
      href={localeHref(locale, `/player/${playerSlug(name, code)}`)}
...
export function MatchTimeline({
  ...
  return (
    <section className="mt-8">
      ...
      <div className="rounded-2xl border border-border bg-card p-4 sm:p-5 dark:inset-ring dark:inset-ring-white/5">
        <div className="mb-1 flex items-center justify-between gap-2 text-xs font-medium text-foreground/80">
        ...
        {shown.length === 0 ? (
          <div className="flex flex-col items-center gap-2 py-8 text-center">
            ...
            <p className="text-sm text-pretty text-muted-2">
              {final && scored
                ? t("match.detailUnavailable")
                : t("match.noEventsYet")}
            </p>
            {hasSubs && !showSubs && (
              <p className="text-xs text-muted-2">{t("match.onlySubs")}</p>
            )}
          </div>
        ) : (
          <ol className="relative mt-2">
            ...
            {shown.map(({ e, score, onHome }, i) => (
              <li
                key={i}
                className="grid grid-cols-[1fr_3rem_1fr] items-center gap-2 py-2 sm:grid-cols-[1fr_3.5rem_1fr] sm:gap-3"
              >
                ...
                <div className="relative z-10 flex flex-col items-center gap-1 bg-card py-0.5">
                  <span className="font-mono text-[10px] whitespace-nowrap text-muted-2 tabular-nums">
                    {e.minute}
                  </span>
                  {score ? (
                    <span className="rounded-md border border-border-strong bg-surface-raised px-1.5 py-0.5 font-mono text-[11px] font-bold text-foreground tabular-nums">
                      {score}
                    </span>
                  ) : (
                    <span
...
function Event({
  ...
  const avatar = e.teamCode
    ? playerImages?.[`${e.teamCode}|${e.player}`]
    : undefined
  const tag =
    e.goalType === "penalty"
      ? t("match.penaltyTag")
  ...
  const icon =
    e.kind === "goal" ? (
      ...
      <span
        ...
        aria-label={
          e.card === "red"
            ? t("match.redCardLabel")
            : t("match.yellowCardLabel")
        }
      />
    ) : (
  ...
  return (
    <div
      className={`flex min-w-0 flex-col gap-0.5 ${home ? "items-end" : "items-start"}`}
    >
      <div
        className={`flex max-w-full min-w-0 items-center gap-2 ${home ? "flex-row" : "flex-row-reverse"}`}
      >
        {avatar ? (
          ...
          <img
            ...
            loading="lazy"
            decoding="async"
            className="size-7 shrink-0 rounded-full border border-border bg-muted object-cover object-top"
          />
        ) : null}
        <div
          className={`min-w-0 truncate text-sm ${home ? "text-right" : "text-left"}`}
        >
          <PlayerName
            name={e.player}
            code={e.teamCode}
            locale={locale}
            className={
              e.kind === "goal" ? "font-semibold" : "text-foreground/90"
            }
          />
          {tag && (
            <span className="ms-1 font-mono text-[10px] tracking-wide text-muted-2 uppercase">
              ({tag})
            </span>
          )}
        </div>
      ...
      {e.assist && (
        <div
          className={`max-w-full min-w-0 truncate text-[11px] text-muted-2 ${home ? "text-right" : "text-left"}`}
        >
          <LinkedLine
            template={t("match.assist", { name: NAME_SLOT })}
            name={e.assist}
            code={e.teamCode}
            locale={locale}
          />
      ...
      {e.playerOff && (
        ...
        >
          <LinkedLine
            template={t("match.subOff", { name: NAME_SLOT })}
            name={e.playerOff}
            code={e.teamCode}
            locale={locale}
          />
...
function GoalIcon({
  ...
  return (
    ...
    >
      ...
      <path
        d="M12 8.2l3.8 2.76-1.45 4.47h-4.7L8.2 10.96z"
        fill="currentColor"
      />
      <path
