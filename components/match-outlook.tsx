...
const LADDER: [RoundKey, string][] = [
  ["advance", "rounds.shortR32"],
  ["r16", "rounds.shortR16"],
  ["qf", "rounds.shortQF"],
  ["sf", "rounds.shortSF"],
  ["final", "rounds.shortFinal"],
  ["title", "rounds.shortChamp"],
]
...
const heat = (v: number) =>
  `color-mix(in oklab, var(--primary) ${Math.round(Math.min(v, 1) * 22)}%, transparent)`

export async function MatchOutlook({
  round,
  home,
  away,
  matches,
    ...
    return (
      <div className="contents">
        <div className="flex items-center gap-2 bg-card px-3 py-2.5">
          ...
          <span
            className={`truncate text-[13px] font-medium ${out ? "text-muted-foreground line-through" : ""}`}
          >
            {tm.name}
          </span>
        </div>
        {out ? (
          ...
          <div
            className="flex items-center bg-card px-3 py-2.5 text-xs text-muted-2"
          ...
          cols.map(([k]) => {
            ...
            return (
              <div
                key={k}
                className={`flex items-center justify-center bg-card py-2.5 font-mono text-[11px] font-bold tabular-nums sm:text-xs ${clinched ? "text-win" : k === "title" ? "text-primary" : ""}`}
  ...
  return (
    <section className="mt-8">
      <h2 className="mb-3 font-mono text-xs font-semibold tracking-[0.1em] text-muted-foreground uppercase">
        {t("match.tournamentOutlook")}
      </h2>
      <div className="overflow-x-auto rounded-2xl border border-border dark:inset-ring dark:inset-ring-white/5">
        <div
          className="grid min-w-[340px] gap-px sm:min-w-[420px]"
          style={{
            gridTemplateColumns: `minmax(92px,1.3fr) repeat(${cols.length}, 1fr)`,
          }}
        >
          ...
          {cols.map(([k, label]) => (
            <div
              key={k}
              className={`flex items-center justify-center bg-card py-2 text-[10px] font-semibold tracking-wide text-muted-2 uppercase ${k === "title" ? "text-primary" : ""}`}
            >
              {t(label)}
            </div>
          ))}
      ...
      <p className="mt-2 text-xs text-muted-2">{t("match.outlookNote")}</p>
    </section>
  )
}
