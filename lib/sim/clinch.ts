...
export function minThirdPlacePoints(
  codes: string[],
  matches: GroupMatch[]
): number {
...
export function maxThirdPlacePoints(
  codes: string[],
  matches: GroupMatch[]
): number {
  const base: Record<string, number> = {}
...
export function maxReachablePoints(
  code: string,
  matches: GroupMatch[]
): number {
  let pts = 0,
  ...
  for (const m of matches) {
    ...
    if (m.played && m.homeGoals != null && m.awayGoals != null) {
      if (m.homeGoals === m.awayGoals) {
        if (involved) pts += 1
      } else if ((m.homeGoals > m.awayGoals ? m.home : m.away) === code)
        pts += 3
...
function h2hTiers(group: string[], out: MatchOutcome[]): string[][] {
  ...
  const distinct = [...new Set(group.map((c) => pts.get(c)!))].sort(
    (a, b) => b - a
  )
...
function tierRank(
  codes: string[],
  out: MatchOutcome[]
): Map<string, { tier: number; tierSize: number }> {
  ...
  tiers.forEach((t, i) =>
    t.forEach((c) => rank.set(c, { tier: i, tierSize: t.length }))
  )
  return rank
}
...
function decidedClinch(codes: string[], matches: GroupMatch[]): GroupClinch {
  ...
  for (const c of codes) {
    const certainlyAbove = codes.filter(
      (o) =>
        o !== c && posA.get(o)! < posA.get(c)! && posB.get(o)! < posB.get(c)!
    ).length
...
export function computeClinch(
  ...
  const canWin = new Set<string>(),
    canNotWin = new Set<string>()
  const canTop2 = new Set<string>(),
    canMiss = new Set<string>()
  const canTop3 = new Set<string>(),
    canBe4th = new Set<string>()

  const playedOut: MatchOutcome[] = played.map((m) => ({
    home: m.home,
    away: m.away,
    o: playedOutcome(m),
  }))
