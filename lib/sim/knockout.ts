...
export interface KOPlayed {
  ...
  winner: string
  homePens?: number
  awayPens?: number // shootout tally (oriented to the bracket's home/away) when the tie went to penalties
}
...
export function resolveKnockoutResults(
  ...
  losers: Record<number, string>
  played: Record<number, KOPlayed>
} {
  const r32 = buildR32(groups, slotToTeam)
  const byPair = new Map(
    results.map((r) => [[r.homeCode, r.awayCode].sort().join("-"), r])
  )
  ...
  const ref = (s: string): string | undefined =>
    s.startsWith("W")
      ? winners[Number(s.slice(1))]
      : s.startsWith("L")
        ? losers[Number(s.slice(1))]
        : undefined
  for (const m of KNOCKOUT) {
    ...
    const winner =
      r.winnerCode === home || r.winnerCode === away
        ? r.winnerCode!
        : homeScore >= awayScore
          ? home
          : away
    ...
    const pens =
      r.homePens != null && r.awayPens != null
        ? {
            homePens: sameOrient ? r.homePens : r.awayPens,
            awayPens: sameOrient ? r.awayPens : r.homePens,
          }
        : {}
...
function playKO(
  ...
  const diff =
    (ratings[homeCode] ?? 1500) -
    (ratings[awayCode] ?? 1500) +
    hostEloBoost(homeCode, venue) -
    hostEloBoost(awayCode, venue)
...
export function simulateKnockout(
  ...
  for (const m of KNOCKOUT) {
    ...
    if (m.round === "R16" || m.round === "QF" || m.round === "SF") {
      reached[m.round].add(home)
      reached[m.round].add(away)
    }
    if (m.round === "F") {
      reached.F.add(home)
      reached.F.add(away)
    ...
    if (dw === home || dw === away) {
      ...
      if (lk) {
        ...
        let diff =
          (ratings[home] ?? 1500) -
          (ratings[away] ?? 1500) +
          hostEloBoost(home, venue) -
          hostEloBoost(away, venue)
  ...
  const semifinalists = [
    ...new Set(sfMatches.flatMap((mn) => [winners[mn], losers[mn]])),
  ]
