...
export function roundRobin(group: string, codes: string[]): GroupMatch[] {
  ...
  for (let i = 0; i < codes.length; i++)
    for (let j = i + 1; j < codes.length; j++)
      ms.push({ group, home: codes[i], away: codes[j], played: false })
  return ms
}
...
export function runMonteCarlo(
  ...
  for (let it = 0; it < iterations; it++) {
    ...
    for (const c in teams)
      pr[c] = (ratings[c] ?? 1500) + gaussian(rand) * RATING_SIGMA
    ...
    const { advancingByGroup, slotToTeam } = selectAndAssignThirds(
      thirds,
      ratings
    )
  ...
  for (const mn in matchAgg) {
    ...
    for (const c in matchAgg[m].home)
      matchProjection[m].home[c] = matchAgg[m].home[c] / N
    for (const c in matchAgg[m].away)
      matchProjection[m].away[c] = matchAgg[m].away[c] / N
  }
