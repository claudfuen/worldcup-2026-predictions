...
export function selectAndAssignThirds(
  thirds: ThirdTeam[],
  ratings: Ratings
): ThirdAssignment {
  ...
  if (!assignment)
    throw new Error(`No third-place table entry for combination "${key}"`)
...
export function lockedThirdSlots(
  ...
  for (const [key, assignment] of Object.entries(THIRD_PLACE_TABLE)) {
    ...
    for (const g of guaranteedGroups)
      slotsSeen[g].add(TP_SLOT_ORDER[assignment.indexOf(g)])
  }
