...
export function liveEloAdjustment(g: LiveInGame, frac: number): number {
  ...
  const redMag = Math.sign(netRed) * (Math.abs(netRed) <= 1 ? Math.abs(netRed) : 1 + 0.6 * (Math.abs(netRed) - 1));
  ...
  const offTargetDiff = ((g.shotsHome ?? 0) - (g.sotHome ?? 0)) - ((g.shotsAway ?? 0) - (g.sotAway ?? 0));
  const possDiff = ((g.possHome ?? 50) - (g.possAway ?? 50)) / 100;
  let dom = Math.max(-170, Math.min(170, sotDiff * 24 + offTargetDiff * 4 + possDiff * 45));
  ...
  if (netRed !== 0 && dom !== 0 && Math.sign(dom) === Math.sign(netRed)) dom *= 0.5;
