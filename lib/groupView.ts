...
function nextMatchNeed(
  code: string,
  codes: string[],
  gm: GroupMatch[]
): string | undefined {
  const rem = gm.filter(
    (m) => !m.played && (m.home === code || m.away === code)
  )
  ...
  const variant = (res: "W" | "D" | "L"): GroupMatch[] =>
    gm.map((g) => {
      if (g !== m) return g
      const hg =
        res === "D" ? 0 : xHome ? (res === "W" ? 1 : 0) : res === "W" ? 0 : 1
      const ag =
        res === "D" ? 0 : xHome ? (res === "W" ? 0 : 1) : res === "W" ? 1 : 0
  ...
  if (cW.top2)
    return lossOut
      ? `Beat ${opp} to go through - a loss is out.`
      : `Beat ${opp} to lock a top-2 spot.`
...
export function buildGroupViews(
  ...
  const settledThirdByGroup: Record<
    string,
  ...
  const groups: GroupView[] = GROUPS.map((g) => {
    ...
    return {
      ...
      teams: rows.map((r) => {
        ...
        const status: GroupTeamView["status"] = cl.winner
          ? "won_group"
          : cl.second
            ? "second"
            : cl.top2 || advancedByThird
              ? "advanced"
              : eliminated
                ? "eliminated"
                : "live"
        ...
        return {
          ...
          winGroup: p.winGroup,
          advance: p.advance,
          advanceDelta: p.advanceDelta,
          status,
          need:
            status === "live"
              ? nextMatchNeed(r.code, codes, clinchMatches[g])
              : undefined,
        }
      }),
    }
...
export function lockedSlotsFromGroups(
  groups: GroupView[]
