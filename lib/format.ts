...
export function zoneLabel(utc: string, z?: Zone): string {
  const parts = new Intl.DateTimeFormat(localeOf(z), {
    timeZone: tzOf(z),
    timeZoneName: "short",
...
export function fmtTime(utc: string, z?: Zone): string {
  return (
    new Date(utc).toLocaleString(localeOf(z), {
      timeZone: tzOf(z),
      hour: "numeric",
      minute: "2-digit",
    }) + ` ${zoneLabel(utc, z)}`
...
export function fmtTimeShort(utc: string, z?: Zone): string {
  return new Date(utc).toLocaleString(localeOf(z), {
    timeZone: tzOf(z),
    hour: "numeric",
    minute: "2-digit",
  })
...
export function relativeDay(
  ...
  const diffDays = Math.round(
    (Date.parse(matchKey) - Date.parse(todayKey)) / 86400000
  )
  if (diffDays > 1 && diffDays <= 6)
    return {
      text: new Date(utc).toLocaleString(localeOf(z), {
        timeZone: tz,
        weekday: "long",
      }),
    }
...
const ORD_EN: Record<string, string> = {
  one: "st",
  two: "nd",
  few: "rd",
  other: "th",
