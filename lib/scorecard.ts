...
export function computeMatchAccuracy(
  matches: MatchInfo[]
): MatchAccuracy | null {
  const scored = matches.filter(
    (m) => m.status === "final" && m.probs && m.home && m.away && outcomeOf(m)
  )
  ...
  const rows = scored.map((m) => {
    ...
    const pick = (["home", "draw", "away"] as Outcome[]).reduce(
      (a, b) => (probOf[b] > probOf[a] ? b : a),
      "home"
    )
...
export const preTournamentForecast = cache(
  async (): Promise<Record<string, { advance: number; title: number }>> => {
    const KEY = "scorecard:pretournament:v1"
    if (KV_CONFIGURED) {
      try {
        const c =
          await kvGetJSON<Record<string, { advance: number; title: number }>>(
            KEY
          )
        if (c) return c
      } catch {
    ...
    for (const t of Object.values(sim.teams))
      out[t.code] = { advance: t.advance, title: t.title }
...
export function computeTournamentAccuracy(
  ...
  if (champion) {
    const ranked = Object.entries(pre)
