...
async function fetchPlayerImage(
  ...
  try {
    r = await fetch(
      `${API}/${KEY}/searchplayers.php?p=${encodeURIComponent(name)}`,
      { cache: "no-store" }
    )
  } catch {
  ...
  const sameNation = (d.player ?? []).filter(
    (p) => p.strNationality && norm(p.strNationality) === cn
  )
  const pick =
    sameNation.find((p) => p.strCutout) ??
    sameNation.find((p) => p.strThumb) ??
    null
...
export const getPlayerImage = cache(
  async (name: string, teamCode: string): Promise<string | null> => {
    ...
    if (KV_CONFIGURED) {
      try {
        ...
        if (c && Date.now() - c.at < (c.url ? 14 : 2) * 24 * 60 * 60_000)
          return c.url
      } catch {
    ...
    if (res !== undefined && KV_CONFIGURED)
      await kvSetJSON(key, { at: Date.now(), url: res }).catch(() => {})
    return res ?? null
  }
)
