...
export const getLiveMatches = cache(async (): Promise<LiveMatch[]> => {
  if (KV_CONFIGURED) {
    try {
      const cached = await kvGetJSON<{ at: number; items: LiveMatch[] }>(
        LIVE_FEED_KEY
      )
      if (cached && Date.now() - cached.at < LIVE_FEED_TTL_MS)
        return cached.items
    } catch {
  ...
  try {
    ...
    if (KV_CONFIGURED)
      await kvSetJSON(LIVE_FEED_KEY, { at: Date.now(), items }).catch(() => {})
    return items
  } catch {
    return []
  }
...
export function overlayLive(
  matches: MatchInfo[],
  live: Awaited<ReturnType<typeof getLiveMatches>>
): MatchInfo[] {
  if (!live.length) return matches
  const byPair = new Map(
    live.map((l) => [[l.homeCode, l.awayCode].sort().join("-"), l])
  )
    ...
    if (l.state === "post") {
      ...
      const pens =
        ko && l.homePens != null && l.awayPens != null
          ? {
              homePens: orient ? l.homePens : l.awayPens,
              awayPens: orient ? l.awayPens : l.homePens,
            }
          : {}
      const winner = ko
...
export async function withLiveAdjustments(
  live: LiveMatch[]
): Promise<LiveMatch[]> {
  const inPlay = live.filter(
    (l) => l.state === "in" && l.minute != null && l.eventId
  )
  if (!inPlay.length) return live
  const summaries = await Promise.all(
    inPlay.map((l) =>
      fetchEventSummary(l.eventId, l.homeCode, l.awayCode).catch(() => null)
    )
  )
    ...
    const a = liveEloAdjustment(
      {
        redHome: reds.home,
        redAway: reds.away,
        possHome: s.stats?.home.possession ?? undefined,
        possAway: s.stats?.away.possession ?? undefined,
        shotsHome: s.stats?.home.shots ?? undefined,
        shotsAway: s.stats?.away.shots ?? undefined,
        sotHome: s.stats?.home.shotsOnTarget ?? undefined,
        sotAway: s.stats?.away.shotsOnTarget ?? undefined,
  ...
  return live.map((l) =>
    adj.has(l.eventId) ? { ...l, eloAdj: adj.get(l.eventId) } : l
  )
}
...
export function liveSignature(live: LiveMatch[]): string {
  return live
    .map(
      (l) =>
        `${[l.homeCode, l.awayCode].sort().join("-")}:${l.state}:${l.homeGoals}-${l.awayGoals}:${l.minute ?? "?"}`
    )
    .sort()
    .join("|")
}
...
export function liveMatchProbs(
  m: MatchInfo,
  ratings: Ratings,
  ...
  let diff =
    (ratings[m.home] ?? 1500) -
    (ratings[m.away] ?? 1500) +
    hostEloBoost(m.home, m.venue) -
    hostEloBoost(m.away, m.venue)
  if (summary) {
    ...
    diff += liveEloAdjustment(
      {
        redHome: reds.home,
        redAway: reds.away,
        possHome: summary.stats?.home.possession ?? undefined,
        possAway: summary.stats?.away.possession ?? undefined,
        shotsHome: summary.stats?.home.shots ?? undefined,
        shotsAway: summary.stats?.away.shots ?? undefined,
        sotHome: summary.stats?.home.shotsOnTarget ?? undefined,
        sotAway: summary.stats?.away.shotsOnTarget ?? undefined,
...
export function attachLiveProbs(
  matches: MatchInfo[],
  ratings: Ratings
): MatchInfo[] {
  return matches.map((m) => {
    ...
    return lp
      ? { ...m, liveProbs: { home: lp.home, draw: lp.draw, away: lp.away } }
      : m
  })
}
...
export function selectTickerItems(matches: MatchInfo[]): MatchInfo[] {
  ...
  const upcoming = matches
    ...
    .slice(0, 5)
  const allFinals = matches
    .filter((m) => m.status === "final")
    .sort((a, b) => b.utc.localeCompare(a.utc))
  const recent = allFinals.filter(
    (m) => now - Date.parse(m.utc) < TICKER_RECENT_MS
  )
