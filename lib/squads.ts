...
interface EspnTeamsList {
  sports?: {
    leagues?: { teams?: { team?: { id?: string; displayName?: string } }[] }[]
  }[]
}
...
async function fetchSquadPositions(): Promise<SquadPositions> {
  ...
  const list = (await (
    await fetch(TEAMS_LIST, { cache: "no-store" })
  ).json()) as EspnTeamsList
  const teams = list.sports?.[0]?.leagues?.[0]?.teams ?? []
  const idToCode: [string, string][] = []
  for (const t of teams) {
    const id = t.team?.id
    const code = t.team?.displayName
      ? TEAM_BY_ESPN[t.team.displayName]?.code
      : undefined
  ...
  await Promise.all(
    idToCode.map(async ([id, code]) => {
      try {
        const r = (await (
          await fetch(ROSTER(id), { cache: "no-store" })
        ).json()) as EspnRoster
        for (const a of flattenAthletes(r.athletes)) {
          const name = a.displayName
          const pos = positionGroup(
            a.position?.name ?? a.position?.abbreviation
          )
...
export const getSquadPositions = cache(async (): Promise<SquadPositions> => {
  if (KV_CONFIGURED) {
    try {
      const c = await kvGetJSON<{ at: number; p: SquadPositions }>(
        SQUAD_POS_KEY
      )
  ...
  if (KV_CONFIGURED && Object.keys(p).length)
    await kvSetJSON(SQUAD_POS_KEY, { at: Date.now(), p }).catch(() => {})
  return p
})
