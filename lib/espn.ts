...
const VENUE_BY_PAIR = new Map<string, string>(
  SCHEDULE.filter((m) => m.round === "GROUP" && m.home && m.away).map((m) => [
    [m.home!, m.away!].sort().join("-"),
    m.venue,
  ])
)
...
export const GROUP_STAGE_END = SCHEDULE.reduce(
  (mx, m) =>
    m.round === "GROUP" && m.utc.slice(0, 10) > mx ? m.utc.slice(0, 10) : mx,
  "2026-06-11"
)
...
function shootout(
  c: { shootoutScore?: string | number } | undefined
): number | null {
...
export function parseLiveMinute(
  displayClock?: string,
  detail?: string,
  period?: number
): number | null {
  ...
  if (
    /full.?time|\bft\b|\baet\b|\bpens?\b|penalt|after extra|\bend\b/.test(lower)
  )
    return 90 // regulation done
    ...
    out.push({
      ...
      minute:
        state === "post"
          ...
          : parseLiveMinute(
              comp.status?.displayClock,
              comp.status?.type?.shortDetail ?? comp.status?.type?.detail,
              comp.status?.period
            ),
      eventId: e.id,
      winnerCode: home.winner ? ht.code : away.winner ? at.code : null,
...
export function preMatchRatingsByPair(
  results: FetchedMatch[]
    ...
    groups[g] = roundRobin(g, codes).map((fixture) => {
      ...
      if (l) {
        ...
        if (l.minute != null) {
          ...
          const eloAdj =
            l.eloAdj != null ? (sameOrient ? l.eloAdj : -l.eloAdj) : undefined
          return {
            ...fixture,
            venue,
            live: {
