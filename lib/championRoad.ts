...
export function applyChampionRoad(
  matches: MatchInfo[],
  ...
  while (m) {
    ...
    if (m.home === champion) side = "home"
      ...
      const ph = m.home
        ? 0
        : (m.projHome?.find((c) => c.code === champion)?.prob ?? 0)
      const pa = m.away
        ? 0
        : (m.projAway?.find((c) => c.code === champion)?.prob ?? 0)
      ...
      if (list && i > 0)
        m[key] = [list[i], ...list.slice(0, i), ...list.slice(i + 1)]
    }
