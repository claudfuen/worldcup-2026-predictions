...
import {
  rankThirds,
  selectAndAssignThirds,
  lockedThirdSlots,
  type ThirdTeam,
} from "./sim/thirdPlace"
import {
  resolveKnockoutResults,
  type GroupOutcome,
  type KOPlayed,
  type KOLive,
} from "./sim/knockout"
...
): Pick<BaselineSnapshot, "title" | "advance"> {
...
export function applyDeltas(
  ...
  for (const g of p.groups) {
    for (const t of g.teams) {
      if (base.advance[t.code] != null)
        t.advanceDelta = t.advance - base.advance[t.code]
    }
  }
}
...
function fillMatchForecast(
  info: MatchInfo,
  ratings: Record<string, number>,
  preMatch: Record<string, Record<string, number>>
): void {
  ...
  const r =
    info.status === "final"
      ? (preMatch[[info.home, info.away].sort().join("-")] ?? ratings)
      : ratings
  const diff =
    (r[info.home] ?? 1500) -
    (r[info.away] ?? 1500) +
    hostEloBoost(info.home, info.venue) -
    hostEloBoost(info.away, info.venue)
  const p = wdlProbs(diff)
  info.probs = { home: p.win, draw: p.draw, away: p.loss }
  const favCode = p.win >= p.loss ? info.home : info.away
  info.favorite = {
    code: favCode,
    name: TEAM_BY_CODE[favCode].name,
  ...
  if (info.status !== "final") {
    info.topScores = scorelineDist(diff)
      .slice(0, 6)
      .map((s) => ({ h: s.h, a: s.a, prob: Math.round(s.prob * 1000) / 1000 }))
  }
}
...
export async function computePredictions(
  ...
  if (GROUPS.every((g) => groupMatches[g].every((m) => m.played))) {
    ...
    const koResults = results.filter(
      (r) => r.group == null || r.date.slice(0, 10) > GROUP_STAGE_END
    )
    const resolved = resolveKnockoutResults(
      groupOutcome,
      selectAndAssignThirds(thirds, ratings).slotToTeam,
      koResults
    )
  ...
  for (const [mn, code] of Object.entries(koWinners))
    koSlotTeam[`W${mn}`] = code
  ...
  for (const l of live) {
    ...
    if (
      l.state !== "in" ||
      l.minute == null ||
      (l.group != null && l.date.slice(0, 10) <= GROUP_STAGE_END)
    )
      continue
    koLive[[l.homeCode, l.awayCode].sort().join("-")] = {
      homeCode: l.homeCode,
      homeScore: l.homeGoals,
      awayScore: l.awayGoals,
      frac: fracRemaining(l.minute),
      eloAdj: l.eloAdj,
    }
  }
  ...
  const teams: TeamPrediction[] = Object.values(sim.teams)
    .map((t) => {
      ...
      return {
        ...t,
        name: TEAM_BY_CODE[t.code].name,
        rating: Math.round(exact),
        ratingExact: exact,
      }
    })
  ...
  const { groups, thirdRows } = buildGroupViews(
    groupMatches,
    ratings,
    (code) => {
  ...
  for (const code in sim.r32Opponents) {
    r32Opponents[code] = Object.entries(sim.r32Opponents[code])
      .map(([opp, prob]) => ({
        code: opp,
        name: TEAM_BY_CODE[opp]?.name ?? opp,
        prob,
      }))
      .sort((a, b) => b.prob - a.prob)
      .slice(0, 6)
  ...
  const matches: MatchInfo[] = SCHEDULE.map((s) => {
    ...
    if (s.round === "GROUP") {
      info.home = s.home!
      info.away = s.away!
      info.homeName = TEAM_BY_CODE[s.home!].name
      info.awayName = TEAM_BY_CODE[s.away!].name
      info.defined = true
      ...
      info.slotHome = s.homeSlot
      info.slotAway = s.awaySlot
        ...
        info.topMatchups = Object.entries(pairs)
          .map(([k, prob]) => {
            ...
            return {
              home: h,
              away: a,
              homeName: TEAM_BY_CODE[h]?.name ?? h,
              awayName: TEAM_BY_CODE[a]?.name ?? a,
              prob,
            }
          })
      ...
      if (pl) {
        ...
        info.home = pl.home
        info.away = pl.away
        info.homeName = TEAM_BY_CODE[pl.home].name
        info.awayName = TEAM_BY_CODE[pl.away].name
        info.homeScore = pl.homeScore
        info.awayScore = pl.awayScore
        info.winner = pl.winner
        if (pl.homePens != null && pl.awayPens != null) {
          info.homePens = pl.homePens
          info.awayPens = pl.awayPens
        }
        info.status = "final"
      } else {
        ...
        const rh = s.homeSlot
          ? (lockedSlot[s.homeSlot] ?? koSlotTeam[s.homeSlot])
          : undefined
        const ra = s.awaySlot
          ? (lockedSlot[s.awaySlot] ?? koSlotTeam[s.awaySlot])
          : undefined
        if (rh) {
          info.home = rh
          info.homeName = TEAM_BY_CODE[rh].name
        }
        if (ra) {
          info.away = ra
          info.awayName = TEAM_BY_CODE[ra].name
        }
      }
  ...
  // finalists from M103's loser-slot distributions so the third-place projection stays consistent.
  {
    ...
    if (m103 && m104) {
      ...
      const homeFinalists = new Set(
        [m104.home, m104.projHome?.[0]?.code].filter(Boolean) as string[]
      )
      const awayFinalists = new Set(
        [m104.away, m104.projAway?.[0]?.code].filter(Boolean) as string[]
      )
      m103.projHome = (m103.projHome ?? []).filter(
        (c) => !homeFinalists.has(c.code)
      )
      m103.projAway = (m103.projAway ?? []).filter(
        (c) => !awayFinalists.has(c.code)
      )
      m103.topMatchups = (m103.topMatchups ?? []).filter(
        (mu) => !homeFinalists.has(mu.home) && !awayFinalists.has(mu.away)
      )
    }
  }

  const matchesPlayed = results.filter(
    (r) => r.group != null && r.date.slice(0, 10) <= GROUP_STAGE_END
  ).length
  ...
  for (const g of groups)
    for (const tm of g.teams) statusByCode[tm.code] = tm.status
  ...
  const guaranteedThirdGroups = groups
    .filter((g) => g.teams[2]?.status === "advanced")
    .map((g) => g.group)
  const eliminatedThirdGroups = groups
    .filter((g) => g.teams[2]?.status === "eliminated")
    .map((g) => g.group)
  const lockedThird = lockedThirdSlots(
    guaranteedThirdGroups,
    eliminatedThirdGroups
  )
  ...
  for (const [g, slot] of Object.entries(lockedThird)) {
    ...
    if (gv?.decided && gv.teams[2])
      lockedThirdTeamBySlot[slot] = gv.teams[2].code
  }
  const winnerByGroup: Record<string, string> = {}
  for (const g of groups) {
    const w = g.teams.find((t) => t.status === "won_group")
    if (w) winnerByGroup[g.group] = w.code
  }
  ...
  const thirdPlaceRace: ThirdPlaceEntry[] = rankedThirds.map((t, i) => {
    ...
    return {
      ...
      group: t.group,
      code,
      name: TEAM_BY_CODE[code].name,
      pts: t.row.pts,
      gd: t.row.gd,
      gf: t.row.gf,
      advancing,
      advanceProb: advProb,
      status: statusByCode[code] ?? "live",
      slot,
      match,
      facesGroup: oppGroup,
      slotLocked: lockedSlotFor ? true : undefined,
      opponent: oppCode
        ? { code: oppCode, name: TEAM_BY_CODE[oppCode].name }
        : undefined,
  ...
  const resolvedThirdBySlot = groupStageComplete
    ? slotToTeamMap
    : lockedThirdTeamBySlot
  for (const mi of matches) {
    if (mi.round !== "R32" || mi.status === "final") continue // a played R32 already has its real teams
    const thirdSide = mi.slotHome?.startsWith("3:")
      ? "home"
      : mi.slotAway?.startsWith("3:")
        ? "away"
        : null
    ...
    if (thirdSide === "home") {
      if (!mi.home) {
        mi.home = code
        mi.homeName = TEAM_BY_CODE[code].name
      }
    } else if (!mi.away) {
      mi.away = code
      mi.awayName = TEAM_BY_CODE[code].name
    }
  ...
  const awards = await computeAwards(
    matches,
    sim.teams,
    getMatchSummary,
    squadPositions
  ...
  const complete =
    matches.length > 0 && matches.every((mi) => mi.status === "final")
