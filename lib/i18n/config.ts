...
export const PREFIXED_LOCALE_IDS: readonly Locale[] = LOCALE_IDS.filter(
...
export const ACTIVE_LOCALES: readonly LocaleConfig[] = LOCALES.filter(
  (l) => l.ready
)
export const ACTIVE_LOCALE_IDS: readonly Locale[] = ACTIVE_LOCALES.map(
  (l) => l.id
)
export const ACTIVE_PREFIXED_LOCALE_IDS: readonly Locale[] =
  ACTIVE_LOCALE_IDS.filter((id) => id !== DEFAULT_LOCALE)
