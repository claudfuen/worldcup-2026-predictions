...
export const getMessages = cache(
  async (): Promise<Messages> => loadMessages(await getLocale())
)
...
export const getT = cache(async (): Promise<TFunction> => {
  ...
  return (key, params) => {
    const template =
      lookupMessage(messages, key) ?? lookupMessage(enMessages, key) ?? key
    return formatMessage(template, params, intl)
  }
})

/** Text direction for the current request — for `dir=` and conditional logical styling. */
export const getDir = cache(
  async (): Promise<"ltr" | "rtl"> => localeConfig(await getLocale()).dir
)

/** The active locale's BCP-47 Intl tag, for ad-hoc Intl.* formatting in server components. */
export const getIntlLocale = cache(
  async (): Promise<string> => localeConfig(await getLocale()).intl
)
