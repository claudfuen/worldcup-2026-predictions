...
export async function flagDataUri(
  code?: string | null
): Promise<string | null> {
  ...
  try {
    svg = await readFile(
      join(process.cwd(), "node_modules/flag-icons/flags/4x3", `${iso}.svg`),
      "utf8"
    )
  } catch {
    try {
      const r = await fetch(
        `https://cdn.jsdelivr.net/npm/flag-icons@7/flags/4x3/${iso}.svg`
      )
...
export async function imgDataUri(
