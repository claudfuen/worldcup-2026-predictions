...
export function playerUniverse(awards: Awards): PlayerRef[] {
  ...
  const source: { player: string; teamCode: string }[] = awards.players?.length
    ...
    : [...awards.goldenBoot, ...awards.assists].map((e) => ({
        player: e.player,
        teamCode: e.teamCode,
      }))
  for (const e of source) {
    ...
    if (!seen.has(slug))
      seen.set(slug, { player: e.player, teamCode: e.teamCode, slug })
  }
  return [...seen.values()]
}
...
export function findPlayer(awards: Awards, slug: string): PlayerView | null {
  const info = (awards.players ?? []).find(
    (p) => playerSlug(p.name, p.teamCode) === slug
  )
  const gbIdx = awards.goldenBoot.findIndex(
    (e) => playerSlug(e.player, e.teamCode) === slug
  )
  const asIdx = awards.assists.findIndex(
    (e) => playerSlug(e.player, e.teamCode) === slug
  )
  if (!info && gbIdx < 0 && asIdx < 0) return null
  const award =
    gbIdx >= 0
      ? awards.goldenBoot[gbIdx]
      : asIdx >= 0
        ? awards.assists[asIdx]
        : undefined
  return {
