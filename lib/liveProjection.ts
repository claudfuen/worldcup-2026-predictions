...
export function provisionalGroup(
  group: string,
  groupRows: MatchInfo[],
  ratings: Ratings
  ...
  for (const g of cronGroups)
    for (const t of g.teams)
      prob.set(t.code, {
        winGroup: t.winGroup,
        advance: t.advance,
        advanceDelta: t.advanceDelta,
      })
  return buildGroupViews(
    rankBy,
    ratings,
    (code) => prob.get(code) ?? { winGroup: 0, advance: 0 },
    clinchBy
  ).groups
}
...
export function finalizeBracket(
  matches: MatchInfo[],
  groups: GroupView[],
  ratings: Ratings
): MatchInfo[] {
  ...
  const lockedThirdTeamBySlot: Record<string, string> = {}
  {
    ...
    const eliminated = groups
      ...
      .map((g) => g.group)
    for (const [g, slot] of Object.entries(
      lockedThirdSlots(guaranteed, eliminated)
    )) {
  ...
  for (const m of matches) {
    if (m.round === "GROUP" || m.status !== "final" || !m.home || !m.away)
      continue
    const winner =
      m.winner ??
  ...
  return matches.map((m) => {
    ...
    const lh = m.slotHome
      ? (lockedSlot[m.slotHome] ?? koSlotTeam[m.slotHome])
      : undefined
    const la = m.slotAway
      ? (lockedSlot[m.slotAway] ?? koSlotTeam[m.slotAway])
      : undefined
    if (lh && !home) home = lh
    if (la && !away) away = la
    ...
    if (m.round === "R32") {
      const thirdSide = m.slotHome?.startsWith("3:")
        ? "home"
        : m.slotAway?.startsWith("3:")
          ? "away"
          : null
      if (thirdSide) {
        const hostSlot = thirdSide === "home" ? m.slotAway : m.slotHome
        const code = hostSlot
          ? (thirdSlotToTeam[hostSlot] ?? lockedThirdTeamBySlot[hostSlot])
          : undefined
        if (code) {
...
export function liveThirdPlaceRace(
  ...
  const meta = new Map<
    ...
    {
      advance: number
      status: GroupView["teams"][number]["status"]
      opponents?: ThirdPlaceEntry["opponents"]
    }
  >()
  for (const g of groups)
    for (const t of g.teams)
      meta.set(t.code, { advance: t.advance, status: t.status })
  for (const e of cronEntries) {
    const m = meta.get(e.code)
    if (m) m.opponents = e.opponents
  }
  ...
  try {
    for (const [slot, code] of Object.entries(
  ...
  return ranked.map((t, i) => {
    ...
    const slot =
      lockedSlotFor ??
      (advancing.has(t.group) ? teamSlot[t.row.code] : undefined)
    const match = slot ? thirdHostMatch[slot] : undefined
    const oppGroup = slot ? slot[1] : undefined
    const oppCode =
      lockedSlotFor && oppGroup ? winnerByGroup[oppGroup] : undefined
    return {
      rank: i + 1,
      group: t.group,
      code: t.row.code,
      ...
      opponent: oppCode
        ? { code: oppCode, name: TEAM_BY_CODE[oppCode]?.name ?? oppCode }
        : undefined,
...
export function ratingsFromTeams(
  teams: { code: string; rating: number; ratingExact?: number }[]
): Ratings {
  const r: Ratings = {}
