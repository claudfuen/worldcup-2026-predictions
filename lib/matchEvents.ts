...
export function positionGroup(name?: string): string {
  ...
  if (s.includes("defender") || s.includes("back") || s.includes("sweeper"))
    return "DF" // incl. wing-back, centre-back, sweeper
...
const espnCode = (name?: string): string | null =>
  name ? (TEAM_BY_ESPN[name]?.code ?? null) : null
  ...
  const side = (code: string): LineupPlayer[] => {
    ...
    return (t?.roster ?? [])
      .map((p) => ({
        ...
        position: positionGroup(p.position?.name ?? p.position?.abbreviation),
        starter: !!p.starter,
        subbedIn: !!p.subbedIn,
...
function parseShootout(
  so: EspnShootoutTeam[] | undefined,
  homeCode: string,
  awayCode: string
): { home: ShootoutKick[]; away: ShootoutKick[] } | null {
...
export function parseKeyEvents(
  ...
  for (const k of keyEvents ?? []) {
    ...
    if (k.scoringPlay) {
      ...
      if (!player) continue
      const goalType = /own goal/i.test(t)
        ? "own"
        : /penalt/i.test(t)
          ? "penalty"
          : "goal"
      const assist =
        goalType === "goal"
          ? k.participants?.[1]?.athlete?.displayName
          : undefined
      out.push({
        kind: "goal",
        minute,
...
function dateWindow(iso: string): string {
  ...
  const fmt = (off: number) =>
    new Date(d.getTime() + off * 86400000)
      .toISOString()
      .slice(0, 10)
      .replace(/-/g, "")
  return `${fmt(-1)}-${fmt(1)}`
...
export async function fetchMatchSummary(
  ...
  const sb = (await (
    await fetch(`${SCOREBOARD}?dates=${dateWindow(utc)}`, { cache: "no-store" })
  ).json()) as {
    events?: {
      id: string
  ...
  for (const e of sb.events ?? []) {
    const codes = (e.competitions?.[0]?.competitors ?? []).map((c) =>
      espnCode(c.team?.displayName)
    )
    if (codes.includes(homeCode) && codes.includes(awayCode)) {
      eventId = e.id
      break
    }
  }
...
export async function fetchEventSummary(
  ...
  const sum = (await (
    await fetch(`${SUMMARY}?event=${eventId}`, { cache: "no-store" })
  ).json()) as {
...
export const getMatchSummary = cache(
  async (m: MatchInfo): Promise<MatchSummary> => {
    ...
    if (KV_CONFIGURED)
      await kvSetJSON(key, { at: Date.now(), s }).catch(() => {})
    return s
  }
)
