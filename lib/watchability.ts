...
export function computeWatchability(
  ...
  const competitiveness = (
    ...
    const rh = ratingOf.get(h) ?? 1500,
      ra = ratingOf.get(a) ?? 1500
  ...
  const isDecider = (m: MatchInfo, h: string, a: string) => {
    ...
    const x = gctx.get(h),
      y = gctx.get(a)
    if (!x || !y) return false
    const contested = (t: { advance: number }) =>
      t.advance > 0.08 && t.advance < 0.95
  ...
  const reasonFor = (
    ...
    const rh = r(h),
      ra = r(a)
    if (dec)
      return { key: "watch.groupDecider", params: { group: m.group ?? "" } }
  ...
  for (const m of matches) {
    ...
    scored.push({
      ...
      score,
      comp,
      reason: reasonFor(m, rp.h, rp.a, comp, dec, host),
      hot: false,
    })
  }
  scored.sort((x, y) => y.score - x.score)
  scored.forEach((p, i) => {
    p.hot = i < HOT_N && p.score >= HOT_FLOOR
  })
  return {
    picks: scored,
    byMatch: new Map(scored.map((p) => [p.match.match, p])),
  }
}
