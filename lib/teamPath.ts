...
export function teamPathToFinal(
  ...
  while (cur) {
    ...
    const mySide: "home" | "away" = inThisRound
      ? resolvedHome
        ? "home"
        : "away"
      : side
    ...
    steps.push({
      ...
      oppCandidates: oppCode
        ? []
        : oppProj
            .slice(0, 3)
            .map((c) => ({ code: c.code, name: c.name, prob: c.prob })),
    })
    ...
    if (round === "FINAL") {
      if (played && teamWon) champion = true
      break
    }

    // Walk to the one match the winner of `cur` feeds into.
    const w: string = `W${cur.match}`
    const next: MatchInfo | undefined = matches.find(
      (m) => m.slotHome === w || m.slotAway === w
    )
    if (!next) break
