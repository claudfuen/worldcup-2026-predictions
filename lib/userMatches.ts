...
export async function getUserMatches(userId: string): Promise<UserMatchRow[]> {
  const { rows } = await pool.query<{ match_no: number; tickets: number | null; note: string | null }>(
    "SELECT match_no, tickets, note FROM user_match WHERE user_id = $1 ORDER BY match_no",
    [userId],
  );
  return rows.map((r) => ({ matchNo: r.match_no, tickets: r.tickets, note: r.note }));
}

// Flag (on=true) or unflag (on=false) a match for a user. Idempotent.
export async function setMatchFlag(userId: string, matchNo: number, on: boolean): Promise<void> {
  if (on) {
    await pool.query(
    ...
    await pool.query("DELETE FROM user_match WHERE user_id = $1 AND match_no = $2", [userId, matchNo]);
  }
}
